...
theorem fekete_multiplicative {y : ℕ → ℝ} (hy_ge1 : ∀ n, 1 ≤ y n)
  ...
  have hx_subadditive : ∀ n m, x (n + m) ≤ x n + x m := by
    ...
    calc x (n + m) = Real.log (y (n + m)) := rfl
      _ ≤ Real.log (y n * y m) :=
          Real.log_le_log (lt_of_lt_of_le one_pos (hy_ge1 (n + m))) (hy_submult n m)
      _ = Real.log (y n) + Real.log (y m) :=
          Real.log_mul (ne_of_gt (lt_of_lt_of_le one_pos (hy_ge1 n)))
            (ne_of_gt (lt_of_lt_of_le one_pos (hy_ge1 m)))
      _ = x n + x m := rfl
  ...
  have heq : ∀ n : ℕ, n ≠ 0 → (y n) ^ (1 / (n : ℝ)) = Real.exp (x n / n) := by
    ...
    have hy_pos : 0 < y n := lt_of_lt_of_le one_pos (hy_ge1 n)
  ...
  have hinf_eq : sInf ((fun n => (y n) ^ (1 / (n : ℝ))) '' {n : ℕ | 1 ≤ n}) =
    ...
    · -- sInf (exp '' S) ≤ exp (sInf S)
      ...
      have hexists := exists_lt_of_csInf_lt hne (lt_add_of_pos_right _ hδ_pos : sInf _ < sInf _ + δ)
