...
theorem asympSubrank_eq_iInf_spectrum (a : S) (ha : p.IsGapped a)
  ...
  · -- inf ≤ asympSubrank a: requires gapped condition
    -- Handle three cases: strictly gapped, equiv to 0, equiv to 1
    rcases ha with ⟨k, hk_pos, hgap⟩ | ha0 | ⟨ha1, h1a⟩
    ...
    · -- Case 2: a ≤_P 0 (a is equivalent to 0)
      ...
      exact p.asympSubrank_nonneg a
    · -- Case 3: a ≡ 1 (a is equivalent to 1)
      -- All spectral points give φ(a) = 1 since φ(1) ≤ φ(a) ≤ φ(1)
      have heq_one : ∀ φ : AsymptoticSpectrum p, AsymptoticSpectrum.eval p a φ = 1 := by
        intro φ
        have h1 : AsymptoticSpectrum.eval p a φ ≤ AsymptoticSpectrum.eval p (1 : S) φ :=
          AsymptoticSpectrum.eval_mono p φ ha1
        have h2 : AsymptoticSpectrum.eval p (1 : S) φ ≤ AsymptoticSpectrum.eval p a φ :=
          AsymptoticSpectrum.eval_mono p φ h1a
        have h3 : AsymptoticSpectrum.eval p (1 : S) φ = 1 := AsymptoticSpectrum.eval_one p φ
        linarith
      simp only [heq_one, ciInf_const]
      -- asympSubrank(a) = 1 when a ≡ 1: use one_le_asympSubrank_le_asympRank
      have h1a' : p.rel (1 : S) a := h1a
      have ⟨h_ge1, h_le_arank⟩ := p.one_le_asympSubrank_le_asympRank h1a'
      have hasub_eq_1 : p.asympSubrank a = 1 := by
        apply le_antisymm
        · -- asympSubrank a ≤ 1: since a ≤_P 1, rank(a) ≤ 1, so asympRank ≤ 1
          have hrank_a_le_1 : p.rank a ≤ 1 := by
            rw [p.rank_le_iff a 1]
            convert ha1 using 1
            simp only [Nat.cast_one]
          calc p.asympSubrank a ≤ p.asympRank a := h_le_arank
            _ ≤ (p.rank a : ℝ) := by
                have hmem : (p.rank (a ^ 1) : ℝ) ^ (1 / (1 : ℝ)) ∈ p.asympRankSet a := by
                  simp only [asympRankSet, Set.mem_image, Set.mem_Ici]
                  exact ⟨1, Nat.le_refl 1, by simp⟩
                simp only [pow_one, one_div, inv_one, Real.rpow_one] at hmem
                exact csInf_le (p.asympRankSet_bddBelow a) hmem
            _ ≤ (1 : ℝ) := by exact_mod_cast hrank_a_le_1
        · exact h_ge1
      rw [hasub_eq_1]
...
theorem asympSubrank_eq_min_spectrum [Nonempty (AsymptoticSpectrum p)] (a : S)
  ...
  rw [asympSubrank_eq_iInf_spectrum p a ha, hinf_eq]
...
theorem le_asympSubrank_mul_of_spectrum [Nonempty (AsymptoticSpectrum p)] (a b : S)
  ...
  rw [asympSubrank_eq_iInf_spectrum p a ha, asympSubrank_eq_iInf_spectrum p b hb,
      asympSubrank_eq_iInf_spectrum p (a * b) hab]
