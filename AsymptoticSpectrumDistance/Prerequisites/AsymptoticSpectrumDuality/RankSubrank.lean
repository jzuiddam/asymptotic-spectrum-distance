...
/-- An element a is "generalized gapped" if either:
    (i) there exists k ∈ ℕ such that a^k ≥_P 2 (strictly gapped), or
    (ii) a is equivalent to 0 (i.e., a ≤_P 0), or
    (iii) a is equivalent to 1 (i.e., a ≤_P 1 and 1 ≤_P a).

    For such elements, asympSubrank(a) = min_{φ ∈ X} φ(a).

    Note: In the graph case, every graph satisfies one of these conditions:
    - Empty graph: equivalent to 0
    - Complete graphs: equivalent to 1
    - Graphs with independent pair: strictly gapped -/
