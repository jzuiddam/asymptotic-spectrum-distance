...
theorem phi_reflects_of_maximal (p : StrassenPreorder S) (hmax : p.IsMaximal)
  ...
  · -- Backward: uses maximality (total + asymptotically closed)
    ...
    · -- If b ≤ a, then φ(b) ≤ φ(a), combined with φ(a) ≤ φ(b) gives equality
      ...
      -- Equal φ values for total preorder should imply a ≈ b (both a ≤ b and b ≤ a).
      -- The tex proof (str_exis) states: "If ≤ is maximal, then a ≤ b ⟺ φ(a) ≤ φ(b)."
      --
      -- The key argument: For maximal preorders, if b ≤ a but a ̸≤ b, then φ(b) < φ(a).
      -- This follows from the inf/sup characterizations:
      --   φ(a) = inf { r/s : s•a ≤ r }
      --   ψ(a) = sup { u/v : u ≤ v•a }
      -- For total preorders, φ = ψ.
      --
      -- If b < a strictly, then for any v ≥ 1 with v•b ≤ v•a (by mul_mono from b ≤ a),
      -- the set { u : u ≤ v•a } strictly contains { u : u ≤ v•b } unless a ≈ b.
      --
      -- More precisely: ψ(b) = sup { u/v : u ≤ v•b } and ψ(a) = sup { u/v : u ≤ v•a }.
      -- If a ̸≤ b, then by maximality, there exists n with n•b ̸≤ n•a + r for some r.
      -- This creates a gap in the sup characterization.
      --
      -- The full proof requires careful analysis of the sup/inf sets.
      -- From heq : φ(a) = φ(b) and hba : b ≤ a, if a ̸≤ b, we'd have φ(b) < φ(a),
      -- contradicting φ(a) = φ(b). Hence a ≤ b.
      --
      -- Key property used: For maximal preorders, the unique spectral point φ is
      -- characterized by: a ≤ b ⟺ φ(a) ≤ φ(b). This is the spectral duality theorem
      -- restricted to a single maximal preorder.
      --
      -- TECHNICAL GAP: Strict separation property.
      --
      -- What we need: If b ≤ a and φ(a) = φ(b), then a ≤ b.
      -- Equivalently (contrapositive): If b ≤ a and a ̸≤ b (strict), then φ(b) < φ(a).
      --
      -- Proof approach:
      -- 1. By maximality, the preorder is total and asymptotically closed.
      -- 2. If a ̸≤ b, then by asymptotic closure, ¬(a ≲ b).
      -- 3. From ¬(a ≲ b), we have: ∃n ≥ 1 such that ¬p.rel (n•a) (n•b + s) for any s.
      -- 4. For the inf characterization: φ(a) = inf { r/s : s ≥ 1, p.rel (s•a) r }
      --    φ(b) = inf { r/s : s ≥ 1, p.rel (s•b) r }
      -- 5. Since p.rel b a but not p.rel a b:
      --    - For every s ≥ 1 with p.rel (s•b) r, we also have p.rel (s•a) r
      --      (by transitivity: s•a ≥ s•b ≥ r in the preorder, giving s•a ≥ r)
      --    - But there exists s such that p.rel (s•a) r for some r, but p.rel (s•b) r
      --      requires a LARGER r due to the strict gap.
      -- 6. This gap in the inf sets gives φ(b) < φ(a).
      --
      -- The formal proof requires careful analysis of how the strict inequality
      -- translates to a gap in the ratio sets. This uses the archimedean property
      -- to find the separating ratio.
      --
      -- Following tex line 776:
      -- "Let a ̸≤ b. From str_props(iv) follows ∃n, na ̸≤ nb + 1.
      --  By totality na ≥ nb + 1. Apply φ to get φ(a) ≥ φ(b) + 1/n."
      --
