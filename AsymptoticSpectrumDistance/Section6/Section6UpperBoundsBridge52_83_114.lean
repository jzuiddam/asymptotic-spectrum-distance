...
-/
/-
# Section 6 Baumert Bridge: α(E_{5/2} ⊠ E_{8/3} ⊠ E_{11/4}) ≤ 11
  (alpha3_52_83_114_le)
...
/-! ## Mixed-multiset Baumert bridge: α(E_{5/2} ⊠ E_{8/3} ⊠ E_{11/4}) ≤ 11

The nested floor bound gives α ≤ 12. The Baumert slicing technique with
WLOG translation by `Z₈ × Z₁₁` (a symmetry of E_{8/3} ⊠ E_{11/4}) bringing
the first element of layer 0 to `(0,0)` rules out α = 12. See
`caseMixed52_83_114_check_true`. -/

/-- Encoding for ordered witness extraction.
Matches `enc83_114_v2` definitionally on `Fin 8 × Fin 11`. -/
private def enc83_114_v2' (v : ZMod 8 × ZMod 11) : ℕ := v.1.val * 11 + v.2.val

private lemma enc83_114_v2'_injective : Function.Injective enc83_114_v2' := by
  intro ⟨a1, a2⟩ ⟨b1, b2⟩ h
  simp only [enc83_114_v2'] at h
...
/-- The fiber of S over a single layer i (5/2 × 8/3 × 11/4 case). -/
private def layerFiber5MB (S : Finset (ZMod 5 × (ZMod 8 × ZMod 11))) (i : ZMod 5) :=
  S.filter (fun p => p.1 = i)

private lemma layerFiber5MB_disjoint (S : Finset (ZMod 5 × (ZMod 8 × ZMod 11)))
    (i j : ZMod 5) (hij : i ≠ j) :
    Disjoint (layerFiber5MB S i) (layerFiber5MB S j) := by
  rw [Finset.disjoint_left]
  intro x hx hy
  simp only [layerFiber5MB, Finset.mem_filter] at hx hy
  exact hij (hx.2 ▸ hy.2)

private lemma layer5MB_sum_eq_card (S : Finset (ZMod 5 × (ZMod 8 × ZMod 11))) :
    ∑ i : ZMod 5, (layerFiber5MB S i).card = S.card := by
  rw [← Finset.card_biUnion]
  · congr 1
    ext x
    simp only [layerFiber5MB, Finset.mem_biUnion, Finset.mem_univ, Finset.mem_filter, true_and]
    exact ⟨fun ⟨_, h⟩ => h.1, fun h => ⟨x.1, h, rfl⟩⟩
  · intro i _ j _ hij
    exact layerFiber5MB_disjoint S i j hij

private lemma pair_filter_eqMB (S : Finset (ZMod 5 × (ZMod 8 × ZMod 11))) (i : ZMod 5) :
    S.filter (fun p => p.1 ∈ ({i, i + 1} : Finset (ZMod 5))) =
    layerFiber5MB S i ∪ layerFiber5MB S (i + 1) := by
  ext x; simp only [layerFiber5MB, Finset.mem_filter, Finset.mem_union,
    Finset.mem_insert, Finset.mem_singleton]
  tauto

private lemma pair_bound_sumMB (S : Finset (ZMod 5 × (ZMod 8 × ZMod 11))) (i : ZMod 5)
    (hfbc : (S.filter (fun p => p.1 ∈ ({i, i + 1} : Finset (ZMod 5)))).card ≤ 5) :
    (layerFiber5MB S i).card + (layerFiber5MB S (i + 1)).card ≤ 5 := by
  rw [pair_filter_eqMB] at hfbc
  rwa [Finset.card_union_of_disjoint
    (layerFiber5MB_disjoint S _ _ (two_distinct_zmod5 i))] at hfbc

/-- Translate S by shifting the first coordinate (5/2 × 8/3 × 11/4 case). -/
private def translateFst5MB (k : ZMod 5) (x : ZMod 5 × (ZMod 8 × ZMod 11)) :
    ZMod 5 × (ZMod 8 × ZMod 11) :=
  (x.1 + k, x.2)

private lemma translateFst5MB_injective (k : ZMod 5) :
    Function.Injective (translateFst5MB k) := by
  intro ⟨a1, a2⟩ ⟨b1, b2⟩ h
  simp only [translateFst5MB, Prod.mk.injEq] at h
  exact Prod.ext (add_right_cancel h.1) h.2

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
/-- First-coordinate translation preserves the IS property. -/
private lemma translateFst5MB_IS (k : ZMod 5)
    (S : Finset (ZMod 5 × (ZMod 8 × ZMod 11)))
...
private lemma translateFst5MB_layerFiber (k : ZMod 5)
    (S : Finset (ZMod 5 × (ZMod 8 × ZMod 11))) (i : ZMod 5) :
    (layerFiber5MB (S.image (translateFst5MB k)) (i + k)).card =
    (layerFiber5MB S i).card := by
  have : layerFiber5MB (S.image (translateFst5MB k)) (i + k) =
      (layerFiber5MB S i).image (translateFst5MB k) := by
    ext ⟨j, a⟩
    simp only [layerFiber5MB, Finset.mem_filter, Finset.mem_image, translateFst5MB]
    constructor
  ...
  rw [this, Finset.card_image_of_injective _ (translateFst5MB_injective k)]

/-- Translate S by shifting the inner two coordinates (Z₈ × Z₁₁ diagonal). -/
private def translateSnd83_114 (δ : ZMod 8 × ZMod 11)
    (x : ZMod 5 × (ZMod 8 × ZMod 11)) : ZMod 5 × (ZMod 8 × ZMod 11) :=
...
private lemma translateSnd83_114_layerFiber (δ : ZMod 8 × ZMod 11)
    (S : Finset (ZMod 5 × (ZMod 8 × ZMod 11))) (i : ZMod 5) :
    (layerFiber5MB (S.image (translateSnd83_114 δ)) i).card =
    (layerFiber5MB S i).card := by
  have : layerFiber5MB (S.image (translateSnd83_114 δ)) i =
      (layerFiber5MB S i).image (translateSnd83_114 δ) := by
    ext ⟨j, a, b⟩
    simp only [layerFiber5MB, Finset.mem_filter, Finset.mem_image, translateSnd83_114]
    constructor
...
/-- Bridge: `crossNonAdj83_114_v2 a b = true` iff the formal adj-or-eq condition fails
    for E_{8/3} ⊠ E_{11/4}. -/
private lemma crossNonAdj83_114_v2_spec (a b : ZMod 8 × ZMod 11) :
    crossNonAdj83_114_v2 a b = true ↔
...
set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
/-- From IS property and first-coordinate adj-or-eq, derive crossNonAdj83_114_v2. -/
private lemma cross_compat_of_IS_5MB
    (S : Finset (ZMod 5 × (ZMod 8 × ZMod 11)))
    (hIS : (strongProduct (fractionGraph 5 2)
      (strongProduct (fractionGraph 8 3) (fractionGraph 11 4))).IsIndepSet ↑S)
    (x y : ZMod 5 × (ZMod 8 × ZMod 11))
    (hx : x ∈ S) (hy : y ∈ S) (hxy : x ≠ y)
    (h1 : x.1 = y.1 ∨ (fractionGraph 5 2).Adj x.1 y.1) :
    crossNonAdj83_114_v2 x.2 y.2 = true := by
  rw [crossNonAdj83_114_v2_spec]
  intro ⟨hc1, hc2⟩
...
private lemma extract_ordered_pair5MB
    (S : Finset (ZMod 5 × (ZMod 8 × ZMod 11))) (i : ZMod 5)
    (hcard : (layerFiber5MB S i).card = 2) :
    ∃ a b : ZMod 8 × ZMod 11,
      (i, a) ∈ S ∧ (i, b) ∈ S ∧ a ≠ b ∧ enc83_114_v2' a < enc83_114_v2' b := by
  obtain ⟨⟨i1, a⟩, ⟨i2, b⟩, hne, hfib⟩ := Finset.card_eq_two.mp hcard
  have h1 : (i1, a) ∈ layerFiber5MB S i := hfib ▸ Finset.mem_insert_self _ _
  have h2 : (i2, b) ∈ layerFiber5MB S i :=
    hfib ▸ Finset.mem_insert.mpr (Or.inr (Finset.mem_singleton.mpr rfl))
  rw [layerFiber5MB, Finset.mem_filter] at h1 h2
  obtain ⟨h1m, h1e⟩ := h1; obtain ⟨h2m, h2e⟩ := h2
  subst h1e; subst h2e
  have hab : a ≠ b := fun h => hne (Prod.ext rfl h)
  rcases Nat.lt_or_gt_of_ne (fun h => hab (enc83_114_v2'_injective h)) with hlt | hgt
  · exact ⟨a, b, h1m, h2m, hab, hlt⟩
  · exact ⟨b, a, h2m, h1m, hab.symm, hgt⟩

private lemma extract_ordered_triple5MB
    (S : Finset (ZMod 5 × (ZMod 8 × ZMod 11))) (i : ZMod 5)
    (hcard : (layerFiber5MB S i).card = 3) :
    ∃ a b c : ZMod 8 × ZMod 11,
      (i, a) ∈ S ∧ (i, b) ∈ S ∧ (i, c) ∈ S ∧
      a ≠ b ∧ a ≠ c ∧ b ≠ c ∧
      enc83_114_v2' a < enc83_114_v2' b ∧ enc83_114_v2' b < enc83_114_v2' c := by
  obtain ⟨⟨i1, a⟩, ⟨i2, b⟩, ⟨i3, c⟩, hne12, hne13, hne23, hfib⟩ :=
    Finset.card_eq_three.mp hcard
  have h1 : (i1, a) ∈ layerFiber5MB S i :=
    hfib ▸ Finset.mem_insert_self _ _
  have h2 : (i2, b) ∈ layerFiber5MB S i :=
    hfib ▸ Finset.mem_insert.mpr (Or.inr (Finset.mem_insert_self _ _))
  have h3 : (i3, c) ∈ layerFiber5MB S i :=
    hfib ▸ Finset.mem_insert.mpr (Or.inr (Finset.mem_insert.mpr
      (Or.inr (Finset.mem_singleton.mpr rfl))))
  rw [layerFiber5MB, Finset.mem_filter] at h1 h2 h3
  ...
  have henc_ab := Nat.lt_or_gt_of_ne (fun h => hab (enc83_114_v2'_injective h))
  have henc_ac := Nat.lt_or_gt_of_ne (fun h => hac (enc83_114_v2'_injective h))
  have henc_bc := Nat.lt_or_gt_of_ne (fun h => hbc (enc83_114_v2'_injective h))
...
/-- Mixed-multiset Baumert contradiction: an IS of size 12 in
    E_{5/2} ⊠ E_{8/3} ⊠ E_{11/4} satisfying the edge-fiber bound 5 cannot exist. -/
private lemma caseMixed52_83_114_baumert_contradiction
    (S : Finset (ZMod 5 × (ZMod 8 × ZMod 11)))
    ...
    False := by
  -- IP rotation
  obtain ⟨k, hk⟩ := ip_rotation7 (fun i => (layerFiber5MB S i).card)
    (by change ∑ i, (layerFiber5MB S i).card = 12; rw [layer5MB_sum_eq_card]; exact hScard)
    (fun i => by
      have := pair_bound_sumMB S i (hfbc i)
      convert this using 2)
  -- Translate to canonical sizes via Z₅ shift
  let κ : ZMod 5 := k
  set S₁ := S.image (translateFst5MB κ) with hS₁_def
  have hIS₁ := translateFst5MB_IS κ S hSis
  have hcanon₁ : ∀ j : ZMod 5, (layerFiber5MB S₁ j).card = canonical_sizes7 j := by
    intro j
    have h := translateFst5MB_layerFiber κ S (j - κ)
  ...
  obtain ⟨w00₁, _w01₁, hw00₁_S₁, _, _, _⟩ := extract_ordered_pair5MB S₁ 0
    (by rw [hcanon₁]; rfl)
  -- Translate by δ = (-w00₁.1, -w00₁.2) so that w00 = (0, 0) in the new S
  let δ : ZMod 8 × ZMod 11 := (-w00₁.1, -w00₁.2)
  set S' := S₁.image (translateSnd83_114 δ) with hS'_def
  have hIS' := translateSnd83_114_IS δ S₁ hIS₁
  have hcanon : ∀ j : ZMod 5, (layerFiber5MB S' j).card = canonical_sizes7 j := by
    intro j
    rw [translateSnd83_114_layerFiber δ S₁ j]
    exact hcanon₁ j
  -- Cross-compat
  have hcc := cross_compat_of_IS_5MB S' hIS'
  have hne_layer : ∀ (i j : ZMod 5) (a b : ZMod 8 × ZMod 11),
      i ≠ j → (i, a) ≠ (j, b) :=
    fun _ _ _ _ hij h => hij (congr_arg Prod.fst h)
  have cc : ∀ (i j : ZMod 5) (a b : ZMod 8 × ZMod 11),
      (i, a) ∈ S' → (j, b) ∈ S' → i ≠ j →
      (fractionGraph 5 2).Adj i j →
      crossNonAdj83_114_v2 a b = true :=
    fun i j a b ha hb hij hadj =>
      hcc _ _ ha hb (hne_layer i j a b hij) (Or.inr hadj)
  have cc_eq : ∀ (i : ZMod 5) (a b : ZMod 8 × ZMod 11),
      (i, a) ∈ S' → (i, b) ∈ S' → a ≠ b →
      crossNonAdj83_114_v2 a b = true :=
    fun i a b ha hb hab =>
      hcc _ _ ha hb (fun h => hab (congr_arg Prod.snd h)) (Or.inl rfl)
  -- compatWith83_114_v2 via cc and cc_eq
  have compat_of_list : ∀ (i : ZMod 5) (a : ZMod 8 × ZMod 11)
      (ws : List (ZMod 5 × (ZMod 8 × ZMod 11))),
      (i, a) ∈ S' →
      (∀ w ∈ ws, w ∈ S' ∧ (i ≠ w.1 → (fractionGraph 5 2).Adj i w.1) ∧
        (i = w.1 → a ≠ w.2)) →
      compatWith83_114_v2 a (ws.map Prod.snd) = true := by
    intro i a ws hia hws
    simp only [compatWith83_114_v2, List.all_eq_true, List.mem_map]
  ...
  obtain ⟨w00, w01, hw00, hw01, hne0, hlt0⟩ := extract_ordered_pair5MB S' 0
    (by rw [hcanon]; rfl)
  ...
  have hw00_zero : w00 = (0, 0) := by
    ...
    -- After re-extraction in S', we have w00 ∈ S' with smallest enc83_114_v2'.
    -- Membership: (0, (0, 0)) ∈ S' since (0, w00₁) ∈ S₁ shifts to (0, w00₁ + δ) = (0, 0).
    have h_zero_in_S' : ((0 : ZMod 5), ((0 : ZMod 8), (0 : ZMod 11))) ∈ S' := by
      rw [hS'_def]
      rw [Finset.mem_image]
    ...
    -- Layer 0 has exactly two elements w00, w01 with enc83_114_v2' w00 < enc83_114_v2' w01.
    -- So {w00, w01} = the layer 0 fiber, and (0, 0) must be one of them.
    have hfib0 : layerFiber5MB S' 0 = {(0, w00), (0, w01)} := by
      have hcard0 : (layerFiber5MB S' 0).card = 2 := by rw [hcanon]; rfl
      have hsubset : ({(0, w00), (0, w01)} : Finset (ZMod 5 × (ZMod 8 × ZMod 11))) ⊆
          layerFiber5MB S' 0 := by
        intro x hx
        simp only [Finset.mem_insert, Finset.mem_singleton] at hx
        rcases hx with rfl | rfl
        · show (0, w00) ∈ layerFiber5MB S' 0
          rw [layerFiber5MB, Finset.mem_filter]; exact ⟨hw00, rfl⟩
        · show (0, w01) ∈ layerFiber5MB S' 0
          rw [layerFiber5MB, Finset.mem_filter]; exact ⟨hw01, rfl⟩
      have hpair_card : ({(0, w00), (0, w01)} : Finset (ZMod 5 × (ZMod 8 × ZMod 11))).card = 2 :=
    ...
    have h_zero_in_fib : ((0 : ZMod 5), ((0 : ZMod 8), (0 : ZMod 11))) ∈ layerFiber5MB S' 0 := by
      rw [layerFiber5MB, Finset.mem_filter]; exact ⟨h_zero_in_S', rfl⟩
    ...
    -- Use enc ordering: enc83_114_v2' w00 < enc83_114_v2' w01, and enc83_114_v2' (0, 0) = 0.
    rcases h_zero_in_fib with ⟨_, h00⟩ | ⟨_, h01⟩
    · exact h00.symm
    · -- If w01 = (0, 0), then enc83_114_v2' w01 = 0, so enc83_114_v2' w00 < 0, impossible.
      exfalso
      have : enc83_114_v2' w01 = 0 := by rw [← h01]; rfl
      omega
  -- Continue extracting all witnesses in S'
  obtain ⟨w10, w11, hw10, hw11, hne1, hlt1⟩ := extract_ordered_pair5MB S' 1
    (by rw [hcanon]; rfl)
  obtain ⟨w20, w21, w22, hw20, hw21, hw22, hne20, hne21, hne22, hlt20, hlt21⟩ :=
    extract_ordered_triple5MB S' 2 (by rw [hcanon]; rfl)
  obtain ⟨w30, w31, hw30, hw31, hne3, hlt3⟩ := extract_ordered_pair5MB S' 3
    (by rw [hcanon]; rfl)
  obtain ⟨w40, w41, w42, hw40, hw41, hw42, hne40, hne41, hne42, hlt40, hlt41⟩ :=
    extract_ordered_triple5MB S' 4 (by rw [hcanon]; rfl)
  ...
  -- compatWith83_114_v2 conditions via compat_of_list
  ...
  -- Show caseMixed52_83_114_check = false, contradicting caseMixed52_83_114_check_true.
  -- The hardcoded s00 = (0, 0) in caseMixed52_83_114_check matches w00 = (0, 0) here.
  -- Substitute w00 := (0, 0) throughout.
  subst hw00_zero
  -- Replace `(⟨0, _⟩, ⟨0, _⟩) : Fin 8 × Fin 11` (which appears in
  -- `caseMixed52_83_114_check`) with the term `(0, 0) : ZMod 8 × ZMod 11` (which
  -- we have facts about), via defeq.
  have s00_eq : ((⟨0, by decide⟩, ⟨0, by decide⟩) : Fin 8 × Fin 11) =
      ((0 : ZMod 8), (0 : ZMod 11)) := rfl
  exfalso
  have hf : caseMixed52_83_114_check = false := by
    unfold caseMixed52_83_114_check
    simp only [Bool.not_eq_false']
    apply List.any_of_mem (mem_verts83_114_v2 w01)
    unfold innerSearch_s01_83_114
    have hlt0' : enc83_114_v2 ((0 : ZMod 8), (0 : ZMod 11)) < enc83_114_v2 w01 := hlt0
    ...
    apply List.any_of_mem (mem_verts83_114_v2 w10)
    simp only [cpt_10, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114_v2 w11)
    simp only [show enc83_114_v2 w10 = enc83_114_v2' w10 from rfl,
               show enc83_114_v2 w11 = enc83_114_v2' w11 from rfl,
               decide_eq_true hlt1, cn_10_11, cpt_11, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114_v2 w20)
    simp only [cpt_20, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114_v2 w21)
    simp only [show enc83_114_v2 w20 = enc83_114_v2' w20 from rfl,
               show enc83_114_v2 w21 = enc83_114_v2' w21 from rfl,
               decide_eq_true hlt20, cn_20_21, cpt_21, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114_v2 w22)
    simp only [show enc83_114_v2 w22 = enc83_114_v2' w22 from rfl,
               decide_eq_true hlt21, cn_20_22, cn_21_22, cpt_22, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114_v2 w30)
    simp only [cpt_30, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114_v2 w31)
    simp only [show enc83_114_v2 w30 = enc83_114_v2' w30 from rfl,
               show enc83_114_v2 w31 = enc83_114_v2' w31 from rfl,
               decide_eq_true hlt3, cn_30_31, cpt_31, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114_v2 w40)
    simp only [cpt_40, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114_v2 w41)
    simp only [show enc83_114_v2 w40 = enc83_114_v2' w40 from rfl,
               show enc83_114_v2 w41 = enc83_114_v2' w41 from rfl,
               decide_eq_true hlt40, cn_40_41, cpt_41, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114_v2 w42)
    simp only [show enc83_114_v2 w42 = enc83_114_v2' w42 from rfl,
               decide_eq_true hlt41, cn_40_42, cn_41_42, cpt_42, Bool.true_and]
  exact absurd caseMixed52_83_114_check_true (by rw [hf]; decide)

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
/-- **Mixed multiset upper bound**: α(E_{5/2} ⊠ (E_{8/3} ⊠ E_{11/4})) ≤ 11.

The nested floor bound gives α ≤ 12. The Baumert slicing technique with WLOG
translation by `Z₈ × Z₁₁` (a symmetry of E_{8/3} ⊠ E_{11/4}) bringing the first
element of layer 0 to `(0, 0)` rules out α = 12: layer-assignment search over
E_{8/3} ⊠ E_{11/4} with `s00 = (0, 0)` finds no valid configuration.
See `caseMixed52_83_114_check_true`. -/
theorem alpha3_52_83_114_le :
    (strongProduct (fractionGraph 5 2)
      (strongProduct (fractionGraph 8 3) (fractionGraph 11 4))).indepNum ≤ 11 := by
  by_contra hge; push_neg at hge
  have hle : (strongProduct (fractionGraph 5 2)
      (strongProduct (fractionGraph 8 3) (fractionGraph 11 4))).indepNum ≤ 12 := by
    have h := nested_floor_three 5 2 8 3 11 4
  ...
  have heq : (strongProduct (fractionGraph 5 2)
      (strongProduct (fractionGraph 8 3) (fractionGraph 11 4))).indepNum = 12 := by omega
  ...
  rw [heq] at hSndp
  -- Edge-fiber bound from α(E_{8/3} ⊠ E_{11/4}) = 5
  have hfbc : ∀ i : ZMod 5,
  ...
  exact caseMixed52_83_114_baumert_contradiction S hSndp.isIndepSet hSndp.card_eq hfbc
