...
-/
/-
# Section 6 Baumert Bridge: α(E_{5/2} ⊠ E_{11/4} ⊠ E_{11/4}) ≤ 11
  (alpha3_52_114_114_le)
...
/-! ## Mixed-multiset Baumert bridge: α(E_{5/2} ⊠ E_{11/4} ⊠ E_{11/4}) ≤ 11

The nested floor bound gives α ≤ 12. The Baumert slicing technique with
WLOG translation by `Z₁₁ × Z₁₁` (a symmetry of E_{11/4} ⊠ E_{11/4}) bringing
the first element of layer 0 to `(0,0)` rules out α = 12. See
`caseMixed52_114_114_check_true`. -/

/-- Encoding for ordered witness extraction.
Matches `enc114_114_v2` definitionally on `Fin 11 × Fin 11`. -/
private def enc114_114_v2' (v : ZMod 11 × ZMod 11) : ℕ := v.1.val * 11 + v.2.val

private lemma enc114_114_v2'_injective : Function.Injective enc114_114_v2' := by
  intro ⟨a1, a2⟩ ⟨b1, b2⟩ h
  simp only [enc114_114_v2'] at h
...
/-- The fiber of S over a single layer i (5/2 × 11/4 × 11/4 case). -/
private def layerFiber5MN (S : Finset (ZMod 5 × (ZMod 11 × ZMod 11))) (i : ZMod 5) :=
  S.filter (fun p => p.1 = i)

private lemma layerFiber5MN_disjoint (S : Finset (ZMod 5 × (ZMod 11 × ZMod 11)))
    (i j : ZMod 5) (hij : i ≠ j) :
    Disjoint (layerFiber5MN S i) (layerFiber5MN S j) := by
  rw [Finset.disjoint_left]
  intro x hx hy
  simp only [layerFiber5MN, Finset.mem_filter] at hx hy
  exact hij (hx.2 ▸ hy.2)

private lemma layer5MN_sum_eq_card (S : Finset (ZMod 5 × (ZMod 11 × ZMod 11))) :
    ∑ i : ZMod 5, (layerFiber5MN S i).card = S.card := by
  rw [← Finset.card_biUnion]
  · congr 1
    ext x
    simp only [layerFiber5MN, Finset.mem_biUnion, Finset.mem_univ, Finset.mem_filter, true_and]
    exact ⟨fun ⟨_, h⟩ => h.1, fun h => ⟨x.1, h, rfl⟩⟩
  · intro i _ j _ hij
    exact layerFiber5MN_disjoint S i j hij

private lemma pair_filter_eqMN (S : Finset (ZMod 5 × (ZMod 11 × ZMod 11))) (i : ZMod 5) :
    S.filter (fun p => p.1 ∈ ({i, i + 1} : Finset (ZMod 5))) =
    layerFiber5MN S i ∪ layerFiber5MN S (i + 1) := by
  ext x; simp only [layerFiber5MN, Finset.mem_filter, Finset.mem_union,
    Finset.mem_insert, Finset.mem_singleton]
  tauto

private lemma pair_bound_sumMN (S : Finset (ZMod 5 × (ZMod 11 × ZMod 11))) (i : ZMod 5)
    (hfbc : (S.filter (fun p => p.1 ∈ ({i, i + 1} : Finset (ZMod 5)))).card ≤ 5) :
    (layerFiber5MN S i).card + (layerFiber5MN S (i + 1)).card ≤ 5 := by
  rw [pair_filter_eqMN] at hfbc
  rwa [Finset.card_union_of_disjoint
    (layerFiber5MN_disjoint S _ _ (two_distinct_zmod5 i))] at hfbc

/-- Translate S by shifting the first coordinate (5/2 × 11/4 × 11/4 case). -/
private def translateFst5MN (k : ZMod 5) (x : ZMod 5 × (ZMod 11 × ZMod 11)) :
    ZMod 5 × (ZMod 11 × ZMod 11) :=
  (x.1 + k, x.2)

private lemma translateFst5MN_injective (k : ZMod 5) :
    Function.Injective (translateFst5MN k) := by
  intro ⟨a1, a2⟩ ⟨b1, b2⟩ h
  simp only [translateFst5MN, Prod.mk.injEq] at h
  exact Prod.ext (add_right_cancel h.1) h.2

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
/-- First-coordinate translation preserves the IS property. -/
private lemma translateFst5MN_IS (k : ZMod 5)
    (S : Finset (ZMod 5 × (ZMod 11 × ZMod 11)))
...
private lemma translateFst5MN_layerFiber (k : ZMod 5)
    (S : Finset (ZMod 5 × (ZMod 11 × ZMod 11))) (i : ZMod 5) :
    (layerFiber5MN (S.image (translateFst5MN k)) (i + k)).card =
    (layerFiber5MN S i).card := by
  have : layerFiber5MN (S.image (translateFst5MN k)) (i + k) =
      (layerFiber5MN S i).image (translateFst5MN k) := by
    ext ⟨j, a⟩
    simp only [layerFiber5MN, Finset.mem_filter, Finset.mem_image, translateFst5MN]
    constructor
  ...
  rw [this, Finset.card_image_of_injective _ (translateFst5MN_injective k)]

/-- Translate S by shifting the inner two coordinates (Z₁₁ × Z₁₁ diagonal). -/
private def translateSnd114_114 (δ : ZMod 11 × ZMod 11)
    (x : ZMod 5 × (ZMod 11 × ZMod 11)) : ZMod 5 × (ZMod 11 × ZMod 11) :=
...
private lemma translateSnd114_114_layerFiber (δ : ZMod 11 × ZMod 11)
    (S : Finset (ZMod 5 × (ZMod 11 × ZMod 11))) (i : ZMod 5) :
    (layerFiber5MN (S.image (translateSnd114_114 δ)) i).card =
    (layerFiber5MN S i).card := by
  have : layerFiber5MN (S.image (translateSnd114_114 δ)) i =
      (layerFiber5MN S i).image (translateSnd114_114 δ) := by
    ext ⟨j, a, b⟩
    simp only [layerFiber5MN, Finset.mem_filter, Finset.mem_image, translateSnd114_114]
    constructor
...
/-- Bridge: `crossNonAdj114_114_v2 a b = true` iff the formal adj-or-eq condition fails
    for E_{11/4} ⊠ E_{11/4}. -/
private lemma crossNonAdj114_114_v2_spec (a b : ZMod 11 × ZMod 11) :
    crossNonAdj114_114_v2 a b = true ↔
...
set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
/-- From IS property and first-coordinate adj-or-eq, derive crossNonAdj114_114_v2. -/
private lemma cross_compat_of_IS_5MN
    (S : Finset (ZMod 5 × (ZMod 11 × ZMod 11)))
    (hIS : (strongProduct (fractionGraph 5 2)
      (strongProduct (fractionGraph 11 4) (fractionGraph 11 4))).IsIndepSet ↑S)
    (x y : ZMod 5 × (ZMod 11 × ZMod 11))
    (hx : x ∈ S) (hy : y ∈ S) (hxy : x ≠ y)
    (h1 : x.1 = y.1 ∨ (fractionGraph 5 2).Adj x.1 y.1) :
    crossNonAdj114_114_v2 x.2 y.2 = true := by
  rw [crossNonAdj114_114_v2_spec]
  intro ⟨hc1, hc2⟩
...
private lemma extract_ordered_pair5MN
    (S : Finset (ZMod 5 × (ZMod 11 × ZMod 11))) (i : ZMod 5)
    (hcard : (layerFiber5MN S i).card = 2) :
    ∃ a b : ZMod 11 × ZMod 11,
      (i, a) ∈ S ∧ (i, b) ∈ S ∧ a ≠ b ∧ enc114_114_v2' a < enc114_114_v2' b := by
  obtain ⟨⟨i1, a⟩, ⟨i2, b⟩, hne, hfib⟩ := Finset.card_eq_two.mp hcard
  have h1 : (i1, a) ∈ layerFiber5MN S i := hfib ▸ Finset.mem_insert_self _ _
  have h2 : (i2, b) ∈ layerFiber5MN S i :=
    hfib ▸ Finset.mem_insert.mpr (Or.inr (Finset.mem_singleton.mpr rfl))
  rw [layerFiber5MN, Finset.mem_filter] at h1 h2
  obtain ⟨h1m, h1e⟩ := h1; obtain ⟨h2m, h2e⟩ := h2
  subst h1e; subst h2e
  have hab : a ≠ b := fun h => hne (Prod.ext rfl h)
  rcases Nat.lt_or_gt_of_ne (fun h => hab (enc114_114_v2'_injective h)) with hlt | hgt
  · exact ⟨a, b, h1m, h2m, hab, hlt⟩
  · exact ⟨b, a, h2m, h1m, hab.symm, hgt⟩

private lemma extract_ordered_triple5MN
    (S : Finset (ZMod 5 × (ZMod 11 × ZMod 11))) (i : ZMod 5)
    (hcard : (layerFiber5MN S i).card = 3) :
    ∃ a b c : ZMod 11 × ZMod 11,
      (i, a) ∈ S ∧ (i, b) ∈ S ∧ (i, c) ∈ S ∧
      a ≠ b ∧ a ≠ c ∧ b ≠ c ∧
      enc114_114_v2' a < enc114_114_v2' b ∧ enc114_114_v2' b < enc114_114_v2' c := by
  obtain ⟨⟨i1, a⟩, ⟨i2, b⟩, ⟨i3, c⟩, hne12, hne13, hne23, hfib⟩ :=
    Finset.card_eq_three.mp hcard
  have h1 : (i1, a) ∈ layerFiber5MN S i :=
    hfib ▸ Finset.mem_insert_self _ _
  have h2 : (i2, b) ∈ layerFiber5MN S i :=
    hfib ▸ Finset.mem_insert.mpr (Or.inr (Finset.mem_insert_self _ _))
  have h3 : (i3, c) ∈ layerFiber5MN S i :=
    hfib ▸ Finset.mem_insert.mpr (Or.inr (Finset.mem_insert.mpr
      (Or.inr (Finset.mem_singleton.mpr rfl))))
  rw [layerFiber5MN, Finset.mem_filter] at h1 h2 h3
  ...
  have henc_ab := Nat.lt_or_gt_of_ne (fun h => hab (enc114_114_v2'_injective h))
  have henc_ac := Nat.lt_or_gt_of_ne (fun h => hac (enc114_114_v2'_injective h))
  have henc_bc := Nat.lt_or_gt_of_ne (fun h => hbc (enc114_114_v2'_injective h))
...
/-- Mixed-multiset Baumert contradiction: an IS of size 12 in
    E_{5/2} ⊠ E_{11/4} ⊠ E_{11/4} satisfying the edge-fiber bound 5 cannot exist. -/
private lemma caseMixed52_114_114_baumert_contradiction
    (S : Finset (ZMod 5 × (ZMod 11 × ZMod 11)))
    ...
    False := by
  -- IP rotation
  obtain ⟨k, hk⟩ := ip_rotation7 (fun i => (layerFiber5MN S i).card)
    (by change ∑ i, (layerFiber5MN S i).card = 12; rw [layer5MN_sum_eq_card]; exact hScard)
    (fun i => by
      have := pair_bound_sumMN S i (hfbc i)
      convert this using 2)
  -- Translate to canonical sizes via Z₅ shift
  let κ : ZMod 5 := k
  set S₁ := S.image (translateFst5MN κ) with hS₁_def
  have hIS₁ := translateFst5MN_IS κ S hSis
  have hcanon₁ : ∀ j : ZMod 5, (layerFiber5MN S₁ j).card = canonical_sizes7 j := by
    intro j
    have h := translateFst5MN_layerFiber κ S (j - κ)
  ...
  obtain ⟨w00₁, _w01₁, hw00₁_S₁, _, _, _⟩ := extract_ordered_pair5MN S₁ 0
    (by rw [hcanon₁]; rfl)
  -- Translate by δ = (-w00₁.1, -w00₁.2) so that w00 = (0, 0) in the new S
  let δ : ZMod 11 × ZMod 11 := (-w00₁.1, -w00₁.2)
  set S' := S₁.image (translateSnd114_114 δ) with hS'_def
  have hIS' := translateSnd114_114_IS δ S₁ hIS₁
  have hcanon : ∀ j : ZMod 5, (layerFiber5MN S' j).card = canonical_sizes7 j := by
    intro j
    rw [translateSnd114_114_layerFiber δ S₁ j]
    exact hcanon₁ j
  -- Cross-compat
  have hcc := cross_compat_of_IS_5MN S' hIS'
  have hne_layer : ∀ (i j : ZMod 5) (a b : ZMod 11 × ZMod 11),
      i ≠ j → (i, a) ≠ (j, b) :=
    fun _ _ _ _ hij h => hij (congr_arg Prod.fst h)
  have cc : ∀ (i j : ZMod 5) (a b : ZMod 11 × ZMod 11),
      (i, a) ∈ S' → (j, b) ∈ S' → i ≠ j →
      (fractionGraph 5 2).Adj i j →
      crossNonAdj114_114_v2 a b = true :=
    fun i j a b ha hb hij hadj =>
      hcc _ _ ha hb (hne_layer i j a b hij) (Or.inr hadj)
  have cc_eq : ∀ (i : ZMod 5) (a b : ZMod 11 × ZMod 11),
      (i, a) ∈ S' → (i, b) ∈ S' → a ≠ b →
      crossNonAdj114_114_v2 a b = true :=
    fun i a b ha hb hab =>
      hcc _ _ ha hb (fun h => hab (congr_arg Prod.snd h)) (Or.inl rfl)
  -- compatWith114_114_v2 via cc and cc_eq
  have compat_of_list : ∀ (i : ZMod 5) (a : ZMod 11 × ZMod 11)
      (ws : List (ZMod 5 × (ZMod 11 × ZMod 11))),
      (i, a) ∈ S' →
      (∀ w ∈ ws, w ∈ S' ∧ (i ≠ w.1 → (fractionGraph 5 2).Adj i w.1) ∧
        (i = w.1 → a ≠ w.2)) →
      compatWith114_114_v2 a (ws.map Prod.snd) = true := by
    intro i a ws hia hws
    simp only [compatWith114_114_v2, List.all_eq_true, List.mem_map]
  ...
  obtain ⟨w00, w01, hw00, hw01, hne0, hlt0⟩ := extract_ordered_pair5MN S' 0
    (by rw [hcanon]; rfl)
  ...
  have hw00_zero : w00 = (0, 0) := by
    ...
    -- After re-extraction in S', we have w00 ∈ S' with smallest enc114_114_v2'.
    -- Membership: (0, (0, 0)) ∈ S' since (0, w00₁) ∈ S₁ shifts to (0, w00₁ + δ) = (0, 0).
    have h_zero_in_S' : ((0 : ZMod 5), ((0 : ZMod 11), (0 : ZMod 11))) ∈ S' := by
      rw [hS'_def]
      rw [Finset.mem_image]
    ...
    -- Layer 0 has exactly two elements w00, w01 with enc114_114_v2' w00 < enc114_114_v2' w01.
    -- So {w00, w01} = the layer 0 fiber, and (0, 0) must be one of them.
    have hfib0 : layerFiber5MN S' 0 = {(0, w00), (0, w01)} := by
      have hcard0 : (layerFiber5MN S' 0).card = 2 := by rw [hcanon]; rfl
      have hsubset : ({(0, w00), (0, w01)} : Finset (ZMod 5 × (ZMod 11 × ZMod 11))) ⊆
          layerFiber5MN S' 0 := by
        intro x hx
        simp only [Finset.mem_insert, Finset.mem_singleton] at hx
        rcases hx with rfl | rfl
        · show (0, w00) ∈ layerFiber5MN S' 0
          rw [layerFiber5MN, Finset.mem_filter]; exact ⟨hw00, rfl⟩
        · show (0, w01) ∈ layerFiber5MN S' 0
          rw [layerFiber5MN, Finset.mem_filter]; exact ⟨hw01, rfl⟩
      have hpair_card : ({(0, w00), (0, w01)} : Finset (ZMod 5 × (ZMod 11 × ZMod 11))).card = 2 :=
    ...
    have h_zero_in_fib : ((0 : ZMod 5), ((0 : ZMod 11), (0 : ZMod 11))) ∈ layerFiber5MN S' 0 := by
      rw [layerFiber5MN, Finset.mem_filter]; exact ⟨h_zero_in_S', rfl⟩
    ...
    -- Use enc ordering: enc114_114_v2' w00 < enc114_114_v2' w01, and enc114_114_v2' (0, 0) = 0.
    rcases h_zero_in_fib with ⟨_, h00⟩ | ⟨_, h01⟩
    · exact h00.symm
    · -- If w01 = (0, 0), then enc114_114_v2' w01 = 0, so enc114_114_v2' w00 < 0, impossible.
      exfalso
      have : enc114_114_v2' w01 = 0 := by rw [← h01]; rfl
      omega
  -- Continue extracting all witnesses in S'
  obtain ⟨w10, w11, hw10, hw11, hne1, hlt1⟩ := extract_ordered_pair5MN S' 1
    (by rw [hcanon]; rfl)
  obtain ⟨w20, w21, w22, hw20, hw21, hw22, hne20, hne21, hne22, hlt20, hlt21⟩ :=
    extract_ordered_triple5MN S' 2 (by rw [hcanon]; rfl)
  obtain ⟨w30, w31, hw30, hw31, hne3, hlt3⟩ := extract_ordered_pair5MN S' 3
    (by rw [hcanon]; rfl)
  obtain ⟨w40, w41, w42, hw40, hw41, hw42, hne40, hne41, hne42, hlt40, hlt41⟩ :=
    extract_ordered_triple5MN S' 4 (by rw [hcanon]; rfl)
  ...
  -- compatWith114_114_v2 conditions via compat_of_list
  ...
  -- Show caseMixed52_114_114_check = false, contradicting caseMixed52_114_114_check_true.
  -- The hardcoded s00 = (0, 0) in caseMixed52_114_114_check matches w00 = (0, 0) here.
  -- Substitute w00 := (0, 0) throughout.
  subst hw00_zero
  -- Replace `(⟨0, _⟩, ⟨0, _⟩) : Fin 11 × Fin 11` (which appears in
  -- `caseMixed52_114_114_check`) with the term `(0, 0) : ZMod 11 × ZMod 11` (which
  -- we have facts about), via defeq.
  have s00_eq : ((⟨0, by decide⟩, ⟨0, by decide⟩) : Fin 11 × Fin 11) =
      ((0 : ZMod 11), (0 : ZMod 11)) := rfl
  exfalso
  have hf : caseMixed52_114_114_check = false := by
    unfold caseMixed52_114_114_check
    simp only [Bool.not_eq_false']
    apply List.any_of_mem (mem_verts114_114_v2 w01)
    unfold innerSearch_s01_114_114
    have hlt0' : enc114_114_v2 ((0 : ZMod 11), (0 : ZMod 11)) < enc114_114_v2 w01 := hlt0
    -- v4.29: decide_eq_true no longer fires via simp due to stricter instance matching
    simp only [s00_eq, cn_00_01, Bool.and_true]
    refine (Bool.and_eq_true _ _).mpr ⟨decide_eq_true hlt0', ?_⟩
    apply List.any_of_mem (mem_verts114_114_v2 w10)
    simp only [cpt_10, Bool.true_and]
    apply List.any_of_mem (mem_verts114_114_v2 w11)
    simp only [show enc114_114_v2 w10 = enc114_114_v2' w10 from rfl,
               show enc114_114_v2 w11 = enc114_114_v2' w11 from rfl,
               decide_eq_true hlt1, cn_10_11, cpt_11, Bool.true_and]
    apply List.any_of_mem (mem_verts114_114_v2 w20)
    simp only [cpt_20, Bool.true_and]
    apply List.any_of_mem (mem_verts114_114_v2 w21)
    simp only [show enc114_114_v2 w20 = enc114_114_v2' w20 from rfl,
               show enc114_114_v2 w21 = enc114_114_v2' w21 from rfl,
               decide_eq_true hlt20, cn_20_21, cpt_21, Bool.true_and]
    apply List.any_of_mem (mem_verts114_114_v2 w22)
    simp only [show enc114_114_v2 w22 = enc114_114_v2' w22 from rfl,
               decide_eq_true hlt21, cn_20_22, cn_21_22, cpt_22, Bool.true_and]
    apply List.any_of_mem (mem_verts114_114_v2 w30)
    simp only [cpt_30, Bool.true_and]
    apply List.any_of_mem (mem_verts114_114_v2 w31)
    simp only [show enc114_114_v2 w30 = enc114_114_v2' w30 from rfl,
               show enc114_114_v2 w31 = enc114_114_v2' w31 from rfl,
               decide_eq_true hlt3, cn_30_31, cpt_31, Bool.true_and]
    apply List.any_of_mem (mem_verts114_114_v2 w40)
    simp only [cpt_40, Bool.true_and]
    apply List.any_of_mem (mem_verts114_114_v2 w41)
    simp only [show enc114_114_v2 w40 = enc114_114_v2' w40 from rfl,
               show enc114_114_v2 w41 = enc114_114_v2' w41 from rfl,
               decide_eq_true hlt40, cn_40_41, cpt_41, Bool.true_and]
    apply List.any_of_mem (mem_verts114_114_v2 w42)
    simp only [show enc114_114_v2 w42 = enc114_114_v2' w42 from rfl,
               decide_eq_true hlt41, cn_40_42, cn_41_42, cpt_42, Bool.true_and]
  exact absurd caseMixed52_114_114_check_true (by rw [hf]; decide)

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
/-- **Mixed multiset upper bound**: α(E_{5/2} ⊠ (E_{11/4} ⊠ E_{11/4})) ≤ 11.

The nested floor bound gives α ≤ 12. The Baumert slicing technique with WLOG
translation by `Z₁₁ × Z₁₁` (a symmetry of E_{11/4} ⊠ E_{11/4}) bringing the first
element of layer 0 to `(0, 0)` rules out α = 12: layer-assignment search over
E_{11/4} ⊠ E_{11/4} with `s00 = (0, 0)` finds no valid configuration.
See `caseMixed52_114_114_check_true`. -/
theorem alpha3_52_114_114_le :
    (strongProduct (fractionGraph 5 2)
  ...
  have hle : (strongProduct (fractionGraph 5 2)
    ...
    have h := nested_floor_three 5 2 11 4 11 4
  ...
  have heq : (strongProduct (fractionGraph 5 2)
      (strongProduct (fractionGraph 11 4) (fractionGraph 11 4))).indepNum = 12 := by omega
  ...
  rw [heq] at hSndp
  -- Edge-fiber bound from α(E_{11/4} ⊠ E_{11/4}) = 5
  have hfbc : ∀ i : ZMod 5,
  ...
  exact caseMixed52_114_114_baumert_contradiction S hSndp.isIndepSet hSndp.card_eq hfbc
