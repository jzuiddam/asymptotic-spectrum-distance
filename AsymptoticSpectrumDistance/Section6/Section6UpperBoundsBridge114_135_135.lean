...
-/
/-
# Section 6 Baumert Bridge: α(E_{11/4} ⊠ E_{13/5}²) ≤ 12 (alpha3_114_135_135_le)
...
Bound #9 of paper Theorem 6.9 case 11. The nested floor bound gives α ≤ 13.
Slicing structure identical to bounds #7/#8 with inner E_{11/4} ⊠ E_{13/5}
(143 verts) instead of E_{8/3} ⊠ E_{11/4} (88 verts) or E_{11/4}² (121 verts).
-/


/-- The fiber of S over a single layer i (bound #9, Z₁₃ outer layers,
    inner Z₁₁ × Z₁₃). -/
private def layerFiber13_143 (S : Finset (ZMod 13 × (ZMod 11 × ZMod 13)))
    (i : ZMod 13) :=
  S.filter (fun p => p.1 = i)

private lemma layerFiber13_143_disjoint
    (S : Finset (ZMod 13 × (ZMod 11 × ZMod 13)))
    (i j : ZMod 13) (hij : i ≠ j) :
    Disjoint (layerFiber13_143 S i) (layerFiber13_143 S j) := by
  rw [Finset.disjoint_left]
  intro x hx hy
  simp only [layerFiber13_143, Finset.mem_filter] at hx hy
  exact hij (hx.2 ▸ hy.2)

private lemma layer13_143_sum_eq_card
    (S : Finset (ZMod 13 × (ZMod 11 × ZMod 13))) :
    ∑ i : ZMod 13, (layerFiber13_143 S i).card = S.card := by
  rw [← Finset.card_biUnion]
  · congr 1
    ext x
    simp only [layerFiber13_143, Finset.mem_biUnion, Finset.mem_univ,
      Finset.mem_filter, true_and]
    exact ⟨fun ⟨_, h⟩ => h.1, fun h => ⟨x.1, h, rfl⟩⟩
  · intro i _ j _ hij
    exact layerFiber13_143_disjoint S i j hij

private lemma quintuple_filter_eq13_143
    (S : Finset (ZMod 13 × (ZMod 11 × ZMod 13))) (i : ZMod 13) :
    S.filter (fun p =>
      p.1 ∈ ({i, i + 1, i + 2, i + 3, i + 4} : Finset (ZMod 13))) =
    layerFiber13_143 S i ∪ layerFiber13_143 S (i + 1) ∪
      layerFiber13_143 S (i + 2) ∪ layerFiber13_143 S (i + 3) ∪
      layerFiber13_143 S (i + 4) := by
  ext x; simp only [layerFiber13_143, Finset.mem_filter, Finset.mem_union,
    Finset.mem_insert, Finset.mem_singleton]
  tauto

private lemma quintuple_bound_sum13_143
    (S : Finset (ZMod 13 × (ZMod 11 × ZMod 13))) (i : ZMod 13)
    (hfbc : (S.filter (fun p =>
      p.1 ∈ ({i, i + 1, i + 2, i + 3, i + 4} : Finset (ZMod 13)))).card ≤ 5) :
    (layerFiber13_143 S i).card + (layerFiber13_143 S (i + 1)).card +
    (layerFiber13_143 S (i + 2)).card + (layerFiber13_143 S (i + 3)).card +
    (layerFiber13_143 S (i + 4)).card ≤ 5 := by
  rw [quintuple_filter_eq13_143] at hfbc
  obtain ⟨h01, h02, h03, h04, h12, h13, h14, h23, h24, h34⟩ :=
    five_distinct_zmod13 i
  have d12 : Disjoint (layerFiber13_143 S i) (layerFiber13_143 S (i + 1)) :=
    layerFiber13_143_disjoint S _ _ h01
  have h_u12 := Finset.card_union_of_disjoint d12
  have d3_with_12 :
      Disjoint (layerFiber13_143 S i ∪ layerFiber13_143 S (i + 1))
        (layerFiber13_143 S (i + 2)) := by
    rw [Finset.disjoint_union_left]
    exact ⟨layerFiber13_143_disjoint S _ _ h02,
      layerFiber13_143_disjoint S _ _ h12⟩
  have h_u123 := Finset.card_union_of_disjoint d3_with_12
  have d4_with_123 :
      Disjoint (layerFiber13_143 S i ∪ layerFiber13_143 S (i + 1) ∪
        layerFiber13_143 S (i + 2)) (layerFiber13_143 S (i + 3)) := by
    rw [Finset.disjoint_union_left, Finset.disjoint_union_left]
    refine ⟨⟨?_, ?_⟩, ?_⟩
    · exact layerFiber13_143_disjoint S _ _ h03
    · exact layerFiber13_143_disjoint S _ _ h13
    · exact layerFiber13_143_disjoint S _ _ h23
  ...
  have d5_with_1234 :
    ...
    refine ⟨⟨⟨?_, ?_⟩, ?_⟩, ?_⟩
    · exact layerFiber13_143_disjoint S _ _ h04
    · exact layerFiber13_143_disjoint S _ _ h14
    · exact layerFiber13_143_disjoint S _ _ h24
    · exact layerFiber13_143_disjoint S _ _ h34
  have h_u12345 := Finset.card_union_of_disjoint d5_with_1234
  linarith

/-- Translate S by shifting the inner two coordinates (bound #9,
    Z₁₁ × Z₁₃). -/
private def translateSnd114_135 (δ : ZMod 11 × ZMod 13)
    (x : ZMod 13 × (ZMod 11 × ZMod 13)) : ZMod 13 × (ZMod 11 × ZMod 13) :=
  (x.1, (x.2.1 + δ.1, x.2.2 + δ.2))

private lemma translateSnd114_135_injective (δ : ZMod 11 × ZMod 13) :
    Function.Injective (translateSnd114_135 δ) := by
  intro ⟨a1, a2, a3⟩ ⟨b1, b2, b3⟩ h
  simp only [translateSnd114_135, Prod.mk.injEq] at h
  obtain ⟨h1, h2, h3⟩ := h
  exact Prod.ext h1 (Prod.ext (add_right_cancel h2) (add_right_cancel h3))

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
/-- Inner-coordinate translation preserves the IS property (bound #9). -/
private lemma translateSnd114_135_IS (δ : ZMod 11 × ZMod 13)
    (S : Finset (ZMod 13 × (ZMod 11 × ZMod 13)))
    (hIS : (strongProduct (fractionGraph 13 5)
      (strongProduct (fractionGraph 11 4) (fractionGraph 13 5))).IsIndepSet ↑S) :
    (strongProduct (fractionGraph 13 5)
      (strongProduct (fractionGraph 11 4) (fractionGraph 13 5))).IsIndepSet
      ↑(S.image (translateSnd114_135 δ)) := by
  ...
  simp only [translateSnd114_135] at hne hadj
  ...
  have h_inner_back : (a2, a3) = (b2, b3) ∨ (strongProduct (fractionGraph 11 4)
      (fractionGraph 13 5)).Adj (a2, a3) (b2, b3) := by
    ...
    · right
      ...
      · exact h3_or.imp (fun h => add_right_cancel h)
          (fractionGraph_adj_translate 13 5 a3 b3 δ.2).mp
  exact (SimpleGraph.isIndepSet_iff _).mp hIS
    (Finset.mem_coe.mpr ha) (Finset.mem_coe.mpr hb) hne'
    ⟨hne', h_first, h_inner_back⟩

private lemma translateSnd114_135_layerFiber (δ : ZMod 11 × ZMod 13)
    (S : Finset (ZMod 13 × (ZMod 11 × ZMod 13))) (i : ZMod 13) :
    (layerFiber13_143 (S.image (translateSnd114_135 δ)) i).card =
    (layerFiber13_143 S i).card := by
  have : layerFiber13_143 (S.image (translateSnd114_135 δ)) i =
      (layerFiber13_143 S i).image (translateSnd114_135 δ) := by
    ext ⟨j, a, b⟩
    simp only [layerFiber13_143, Finset.mem_filter, Finset.mem_image,
      translateSnd114_135]
    constructor
  ...
  rw [this, Finset.card_image_of_injective _ (translateSnd114_135_injective δ)]

set_option linter.style.nativeDecide false in
/-- Bridge: `crossNonAdj114_135 a b = true` iff the formal adj-or-eq condition
    fails for E_{11/4} ⊠ E_{13/5}. -/
private lemma crossNonAdj114_135_spec (a b : ZMod 11 × ZMod 13) :
    crossNonAdj114_135 a b = true ↔
    ¬((a.1 = b.1 ∨ (fractionGraph 11 4).Adj a.1 b.1) ∧
      (a.2 = b.2 ∨ (fractionGraph 13 5).Adj a.2 b.2)) := by
...
/-- From IS property and first-coordinate adj-or-eq, derive
    crossNonAdj114_135. -/
private lemma cross_compat_of_IS_114_135
    (S : Finset (ZMod 13 × (ZMod 11 × ZMod 13)))
    (hIS : (strongProduct (fractionGraph 13 5)
      (strongProduct (fractionGraph 11 4) (fractionGraph 13 5))).IsIndepSet ↑S)
    (x y : ZMod 13 × (ZMod 11 × ZMod 13))
    (hx : x ∈ S) (hy : y ∈ S) (hxy : x ≠ y)
    (h1 : x.1 = y.1 ∨ (fractionGraph 13 5).Adj x.1 y.1) :
    crossNonAdj114_135 x.2 y.2 = true := by
  rw [crossNonAdj114_135_spec]
  intro ⟨hc1, hc2⟩
  have hnadj : ¬(strongProduct (fractionGraph 13 5)
      (strongProduct (fractionGraph 11 4) (fractionGraph 13 5))).Adj x y :=
...
/-- Extract the unique witness from a single-element layer (bound #9). -/
private lemma extract_single_from_layer13_143
    (S : Finset (ZMod 13 × (ZMod 11 × ZMod 13))) (i : ZMod 13)
    (hcard : (layerFiber13_143 S i).card = 1) :
    ∃ a : ZMod 11 × ZMod 13, (i, a) ∈ S := by
  obtain ⟨⟨j, a⟩, hja⟩ := Finset.card_pos.mp
    (by omega : 0 < (layerFiber13_143 S i).card)
  rw [layerFiber13_143, Finset.mem_filter] at hja
  exact ⟨a, hja.2 ▸ hja.1⟩

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
set_option linter.style.nativeDecide false in
/-- Bound #9 mixed-multiset Baumert contradiction: an IS of size 13 in
    E_{11/4} ⊠ E_{13/5} ⊠ E_{13/5} satisfying the 5-window fiber bound 5
    cannot exist. -/
private lemma case114_135_135_baumert_contradiction
    (S : Finset (ZMod 13 × (ZMod 11 × ZMod 13)))
    ...
    (hfbc : ∀ i : ZMod 13,
      (S.filter (fun p =>
        p.1 ∈ ({i, i + 1, i + 2, i + 3, i + 4} : Finset (ZMod 13)))).card ≤ 5) :
    False := by
  have hsize : ∀ j : ZMod 13, (layerFiber13_143 S j).card = 1 := by
    apply ip_uniform13 (fun i => (layerFiber13_143 S i).card)
    · rw [layer13_143_sum_eq_card]; exact hScard
    · intro i; exact quintuple_bound_sum13_143 S i (hfbc i)
  obtain ⟨w0₁, hw0₁⟩ := extract_single_from_layer13_143 S 0 (hsize 0)
  let δ : ZMod 11 × ZMod 13 := (-w0₁.1, -w0₁.2)
  set S' := S.image (translateSnd114_135 δ) with hS'_def
  have hIS' := translateSnd114_135_IS δ S hSis
  have hsize' : ∀ j : ZMod 13, (layerFiber13_143 S' j).card = 1 := by
    intro j
    rw [translateSnd114_135_layerFiber δ S j]
    exact hsize j
  have hcc := cross_compat_of_IS_114_135 S' hIS'
  have hne_layer : ∀ (i j : ZMod 13) (a b : ZMod 11 × ZMod 13),
      i ≠ j → (i, a) ≠ (j, b) :=
    fun _ _ _ _ hij h => hij (congr_arg Prod.fst h)
  have cc : ∀ (i j : ZMod 13) (a b : ZMod 11 × ZMod 13),
      (i, a) ∈ S' → (j, b) ∈ S' → i ≠ j →
      (fractionGraph 13 5).Adj i j →
      crossNonAdj114_135 a b = true :=
    fun i j a b ha hb hij hadj =>
      hcc _ _ ha hb (hne_layer i j a b hij) (Or.inr hadj)
  have cc_eq : ∀ (i : ZMod 13) (a b : ZMod 11 × ZMod 13),
      (i, a) ∈ S' → (i, b) ∈ S' → a ≠ b →
      crossNonAdj114_135 a b = true :=
    fun i a b ha hb hab =>
      hcc _ _ ha hb (fun h => hab (congr_arg Prod.snd h)) (Or.inl rfl)
  have compat_of_list : ∀ (i : ZMod 13) (a : ZMod 11 × ZMod 13)
      (ws : List (ZMod 13 × (ZMod 11 × ZMod 13))),
      (i, a) ∈ S' →
      (∀ w ∈ ws, w ∈ S' ∧ (i ≠ w.1 → (fractionGraph 13 5).Adj i w.1) ∧
        (i = w.1 → a ≠ w.2)) →
      compatWith114_135 a (ws.map Prod.snd) = true := by
    intro i a ws hia hws
    simp only [compatWith114_135, List.all_eq_true, List.mem_map]
  ...
  obtain ⟨w0, hw0⟩ := extract_single_from_layer13_143 S' 0 (hsize' 0)
  have h_zero_in_S' : ((0 : ZMod 13), ((0 : ZMod 11), (0 : ZMod 13))) ∈ S' := by
    rw [hS'_def, Finset.mem_image]
    refine ⟨(0, w0₁), hw0₁, ?_⟩
    simp only [translateSnd114_135, δ]
    ext
    · rfl
    · change w0₁.1 + (- w0₁.1) = 0; ring
    · change w0₁.2 + (- w0₁.2) = 0; ring
  have hfib0 : layerFiber13_143 S' 0 = {(0, w0)} := by
    have hcard0 : (layerFiber13_143 S' 0).card = 1 := hsize' 0
    have hsubset : ({(0, w0)} : Finset (ZMod 13 × (ZMod 11 × ZMod 13))) ⊆
        layerFiber13_143 S' 0 := by
      intro x hx
      simp only [Finset.mem_singleton] at hx
      subst hx
      rw [layerFiber13_143, Finset.mem_filter]; exact ⟨hw0, rfl⟩
    have hpair_card : ({(0, w0)} :
        Finset (ZMod 13 × (ZMod 11 × ZMod 13))).card = 1 :=
      Finset.card_singleton _
    exact (Finset.eq_of_subset_of_card_le hsubset
      (by rw [hcard0, hpair_card])).symm
  have h_zero_in_fib : ((0 : ZMod 13), ((0 : ZMod 11), (0 : ZMod 13))) ∈
      layerFiber13_143 S' 0 := by
    rw [layerFiber13_143, Finset.mem_filter]; exact ⟨h_zero_in_S', rfl⟩
  ...
  subst hw0_zero
  obtain ⟨w1, hw1⟩ := extract_single_from_layer13_143 S' 1 (hsize' 1)
  obtain ⟨w2, hw2⟩ := extract_single_from_layer13_143 S' 2 (hsize' 2)
  obtain ⟨w3, hw3⟩ := extract_single_from_layer13_143 S' 3 (hsize' 3)
  obtain ⟨w4, hw4⟩ := extract_single_from_layer13_143 S' 4 (hsize' 4)
  obtain ⟨w5, hw5⟩ := extract_single_from_layer13_143 S' 5 (hsize' 5)
  obtain ⟨w6, hw6⟩ := extract_single_from_layer13_143 S' 6 (hsize' 6)
  obtain ⟨w7, hw7⟩ := extract_single_from_layer13_143 S' 7 (hsize' 7)
  obtain ⟨w8, hw8⟩ := extract_single_from_layer13_143 S' 8 (hsize' 8)
  obtain ⟨w9, hw9⟩ := extract_single_from_layer13_143 S' 9 (hsize' 9)
  obtain ⟨w10, hw10⟩ := extract_single_from_layer13_143 S' 10 (hsize' 10)
  obtain ⟨w11, hw11⟩ := extract_single_from_layer13_143 S' 11 (hsize' 11)
  obtain ⟨w12, hw12⟩ := extract_single_from_layer13_143 S' 12 (hsize' 12)
  ...
  have cpt_2 := compat_of_list 2 w2 [(0, ((0:ZMod 11), (0:ZMod 13))), (1, w1)] hw2
  ...
  have cpt_3 := compat_of_list 3 w3
    [(0, ((0:ZMod 11), (0:ZMod 13))), (1, w1), (2, w2)] hw3
    (by intro w hw; simp only [List.mem_cons, List.mem_nil_iff, or_false] at hw
  ...
  have cpt_4 := compat_of_list 4 w4
    [(0, ((0:ZMod 11), (0:ZMod 13))), (1, w1), (2, w2), (3, w3)] hw4
    (by intro w hw; simp only [List.mem_cons, List.mem_nil_iff, or_false] at hw
  ...
  have cpt_9 := compat_of_list 9 w9
    [(5, w5), (6, w6), (7, w7), (8, w8), (0, ((0:ZMod 11), (0:ZMod 13)))] hw9
    (by intro w hw; simp only [List.mem_cons, List.mem_nil_iff, or_false] at hw
  ...
  have cpt_10 := compat_of_list 10 w10
    [(6, w6), (7, w7), (8, w8), (9, w9),
     (0, ((0:ZMod 11), (0:ZMod 13))), (1, w1)] hw10
    (by intro w hw; simp only [List.mem_cons, List.mem_nil_iff, or_false] at hw
  ...
  have cpt_11 := compat_of_list 11 w11
    [(7, w7), (8, w8), (9, w9), (10, w10),
     (0, ((0:ZMod 11), (0:ZMod 13))), (1, w1), (2, w2)] hw11
    (by intro w hw; simp only [List.mem_cons, List.mem_nil_iff, or_false] at hw
  ...
  have cpt_12 := compat_of_list 12 w12
    [(8, w8), (9, w9), (10, w10), (11, w11),
     (0, ((0:ZMod 11), (0:ZMod 13))), (1, w1), (2, w2), (3, w3)] hw12
    (by intro w hw; simp only [List.mem_cons, List.mem_nil_iff, or_false] at hw
  ...
  have s0_eq : ((⟨0, by decide⟩, ⟨0, by decide⟩) : Fin 11 × Fin 13) =
      ((0 : ZMod 11), (0 : ZMod 13)) := rfl
  exfalso
  have hf : caseMixed114_135_135_check = false := by
    unfold caseMixed114_135_135_check
    simp only [s0_eq, Bool.not_eq_false']
    apply List.any_of_mem (mem_verts114_135 w1)
    simp only [cn_1_0, Bool.true_and]
    apply List.any_of_mem (mem_verts114_135 w2)
    simp only [cpt_2, Bool.true_and]
    apply List.any_of_mem (mem_verts114_135 w3)
    simp only [cpt_3, Bool.true_and]
    apply List.any_of_mem (mem_verts114_135 w4)
    simp only [cpt_4, Bool.true_and]
    apply List.any_of_mem (mem_verts114_135 w5)
    simp only [cpt_5, Bool.true_and]
    apply List.any_of_mem (mem_verts114_135 w6)
    simp only [cpt_6, Bool.true_and]
    apply List.any_of_mem (mem_verts114_135 w7)
    simp only [cpt_7, Bool.true_and]
    apply List.any_of_mem (mem_verts114_135 w8)
    simp only [cpt_8, Bool.true_and]
    apply List.any_of_mem (mem_verts114_135 w9)
    simp only [cpt_9, Bool.true_and]
    apply List.any_of_mem (mem_verts114_135 w10)
    simp only [cpt_10, Bool.true_and]
    apply List.any_of_mem (mem_verts114_135 w11)
    simp only [cpt_11, Bool.true_and]
    apply List.any_of_mem (mem_verts114_135 w12)
    simp only [cpt_12]
  exact absurd caseMixed114_135_135_check_true (by rw [hf]; decide)

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
/-- **Mixed multiset upper bound (bound #9)**:
    α(E_{11/4} ⊠ (E_{13/5} ⊠ E_{13/5})) ≤ 12.

The nested floor bound gives α ≤ 13. The Baumert slicing technique with WLOG
translation by `Z₁₁ × Z₁₃` (a symmetry of E_{11/4} ⊠ E_{13/5}) bringing the
unique element of layer 0 to `(0, 0)` rules out α = 13. See
`caseMixed114_135_135_check_true`. -/
theorem alpha3_114_135_135_le :
    (strongProduct (fractionGraph 11 4)
      (strongProduct (fractionGraph 13 5) (fractionGraph 13 5))).indepNum ≤ 12 := by
  have hbound : (strongProduct (fractionGraph 13 5)
    ...
    have hle : (strongProduct (fractionGraph 13 5)
        (strongProduct (fractionGraph 11 4) (fractionGraph 13 5))).indepNum ≤ 13 := by
      have h := nested_floor_three 13 5 11 4 13 5
    ...
    obtain ⟨S, hSndp⟩ := SimpleGraph.exists_isNIndepSet_indepNum
      (G := strongProduct (fractionGraph 13 5)
        (strongProduct (fractionGraph 11 4) (fractionGraph 13 5)))
    rw [heq] at hSndp
    have hfbc : ∀ i : ZMod 13,
        (S.filter (fun p =>
      ...
      have h := fiber_bound_clique (fractionGraph 13 5)
        ...
        (five_clique_E135 i)
      rwa [alpha_11o4_13o5] at h
    exact case114_135_135_baumert_contradiction S hSndp.isIndepSet
      hSndp.card_eq hfbc
  -- Transfer: α(E_{11/4} ⊠ (E_{13/5} ⊠ E_{13/5}))
  --   = α((E_{11/4} ⊠ E_{13/5}) ⊠ E_{13/5})            (assoc)
  --   = α(E_{13/5} ⊠ (E_{11/4} ⊠ E_{13/5}))            (comm)
