...
-/
/-
# Section 6 Baumert Bridge: α(E_{8/3}² ⊠ E_{11/4}) ≤ 12 (alpha3_83_83_114_le)
...
Bound #5 of paper Theorem 6.9 case 11. Slice by an E_{8/3}: 8 layers in
E_{8/3} ⊠ E_{11/4} (88 verts, α = 5). 3-clique window sum ≤ 5, total = 13,
slack 1. The IP forces sizes to be a rotation of (1,2,1,2,2,1,2,2) (same
canonical pattern as case 8). WLOG translate by `Z₈ × Z₁₁` to bring the
unique element of layer 0 to `(0, 0)`. -/

private def enc83_114'_z (v : ZMod 8 × ZMod 11) : ℕ := v.1.val * 11 + v.2.val

private lemma enc83_114'_z_injective : Function.Injective enc83_114'_z := by
  intro ⟨a1, a2⟩ ⟨b1, b2⟩ h
  simp only [enc83_114'_z] at h
...
/-- The fiber of S over a single layer i (bound #5, Z₈ outer layers,
    inner Z₈ × Z₁₁). -/
private def layerFiber8_88 (S : Finset (ZMod 8 × (ZMod 8 × ZMod 11)))
    (i : ZMod 8) :=
  S.filter (fun p => p.1 = i)

private lemma layerFiber8_88_disjoint
    (S : Finset (ZMod 8 × (ZMod 8 × ZMod 11)))
    (i j : ZMod 8) (hij : i ≠ j) :
    Disjoint (layerFiber8_88 S i) (layerFiber8_88 S j) := by
  rw [Finset.disjoint_left]
  intro x hx hy
  simp only [layerFiber8_88, Finset.mem_filter] at hx hy
  exact hij (hx.2 ▸ hy.2)

private lemma layer8_88_sum_eq_card
    (S : Finset (ZMod 8 × (ZMod 8 × ZMod 11))) :
    ∑ i : ZMod 8, (layerFiber8_88 S i).card = S.card := by
  rw [← Finset.card_biUnion]
  · congr 1
    ext x
    simp only [layerFiber8_88, Finset.mem_biUnion, Finset.mem_univ,
      Finset.mem_filter, true_and]
    exact ⟨fun ⟨_, h⟩ => h.1, fun h => ⟨x.1, h, rfl⟩⟩
  · intro i _ j _ hij
    exact layerFiber8_88_disjoint S i j hij

private lemma triple_filter_eq8_88
    (S : Finset (ZMod 8 × (ZMod 8 × ZMod 11))) (i : ZMod 8) :
    S.filter (fun p => p.1 ∈ ({i, i + 1, i + 2} : Finset (ZMod 8))) =
    layerFiber8_88 S i ∪ layerFiber8_88 S (i + 1) ∪
      layerFiber8_88 S (i + 2) := by
  ext x; simp only [layerFiber8_88, Finset.mem_filter, Finset.mem_union,
    Finset.mem_insert, Finset.mem_singleton]
  tauto

private lemma triple_bound_sum8_88
    (S : Finset (ZMod 8 × (ZMod 8 × ZMod 11))) (i : ZMod 8)
    (hfbc : (S.filter
      (fun p => p.1 ∈ ({i, i + 1, i + 2} : Finset (ZMod 8)))).card ≤ 5) :
    (layerFiber8_88 S i).card + (layerFiber8_88 S (i + 1)).card +
    (layerFiber8_88 S (i + 2)).card ≤ 5 := by
  rw [triple_filter_eq8_88] at hfbc
  obtain ⟨h01, h02, h12⟩ := three_distinct_zmod8 i
  have h1 := Finset.card_union_of_disjoint (layerFiber8_88_disjoint S _ _ h01)
  have h23 :
      Disjoint (layerFiber8_88 S i ∪ layerFiber8_88 S (i + 1))
        (layerFiber8_88 S (i + 2)) := by
    rw [Finset.disjoint_union_left]
    exact ⟨layerFiber8_88_disjoint S _ _ h02, layerFiber8_88_disjoint S _ _ h12⟩
  have h2 := Finset.card_union_of_disjoint h23
  linarith

/-- Translate S by shifting the first coordinate (bound #5, Z₈). -/
private def translateFst8_88 (k : ZMod 8) (x : ZMod 8 × (ZMod 8 × ZMod 11)) :
    ZMod 8 × (ZMod 8 × ZMod 11) :=
  (x.1 + k, x.2)

private lemma translateFst8_88_injective (k : ZMod 8) :
    Function.Injective (translateFst8_88 k) := by
  intro ⟨a1, a2⟩ ⟨b1, b2⟩ h
  simp only [translateFst8_88, Prod.mk.injEq] at h
  exact Prod.ext (add_right_cancel h.1) h.2

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
/-- First-coordinate translation preserves the IS property (bound #5). -/
private lemma translateFst8_88_IS (k : ZMod 8)
    (S : Finset (ZMod 8 × (ZMod 8 × ZMod 11)))
...
private lemma translateFst8_88_layerFiber (k : ZMod 8)
    (S : Finset (ZMod 8 × (ZMod 8 × ZMod 11))) (i : ZMod 8) :
    (layerFiber8_88 (S.image (translateFst8_88 k)) (i + k)).card =
    (layerFiber8_88 S i).card := by
  have : layerFiber8_88 (S.image (translateFst8_88 k)) (i + k) =
      (layerFiber8_88 S i).image (translateFst8_88 k) := by
    ext ⟨j, a⟩
    simp only [layerFiber8_88, Finset.mem_filter, Finset.mem_image,
      translateFst8_88]
    constructor
  ...
  rw [this, Finset.card_image_of_injective _ (translateFst8_88_injective k)]

/-- Translate S by shifting the inner two coordinates (bound #5, Z₈ × Z₁₁). -/
private def translateSnd83_114' (δ : ZMod 8 × ZMod 11)
    (x : ZMod 8 × (ZMod 8 × ZMod 11)) : ZMod 8 × (ZMod 8 × ZMod 11) :=
  (x.1, (x.2.1 + δ.1, x.2.2 + δ.2))

private lemma translateSnd83_114'_injective (δ : ZMod 8 × ZMod 11) :
...
set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
/-- Inner-coordinate translation preserves the IS property (bound #5). -/
private lemma translateSnd83_114'_IS (δ : ZMod 8 × ZMod 11)
    (S : Finset (ZMod 8 × (ZMod 8 × ZMod 11)))
    (hIS : (strongProduct (fractionGraph 8 3)
      (strongProduct (fractionGraph 8 3) (fractionGraph 11 4))).IsIndepSet ↑S) :
    (strongProduct (fractionGraph 8 3)
...
private lemma translateSnd83_114'_layerFiber (δ : ZMod 8 × ZMod 11)
    (S : Finset (ZMod 8 × (ZMod 8 × ZMod 11))) (i : ZMod 8) :
    (layerFiber8_88 (S.image (translateSnd83_114' δ)) i).card =
    (layerFiber8_88 S i).card := by
  have : layerFiber8_88 (S.image (translateSnd83_114' δ)) i =
      (layerFiber8_88 S i).image (translateSnd83_114' δ) := by
    ext ⟨j, a, b⟩
    simp only [layerFiber8_88, Finset.mem_filter, Finset.mem_image,
      translateSnd83_114']
    constructor
  ...
  rw [this, Finset.card_image_of_injective _ (translateSnd83_114'_injective δ)]

set_option linter.style.nativeDecide false in
/-- Bridge: `crossNonAdj83_114' a b = true` iff the formal adj-or-eq condition
    fails for E_{8/3} ⊠ E_{11/4} (bound #5). -/
private lemma crossNonAdj83_114'_spec (a b : ZMod 8 × ZMod 11) :
...
/-- From IS property and first-coordinate adj-or-eq, derive
    crossNonAdj83_114' (bound #5). -/
private lemma cross_compat_of_IS_83_83_114
    (S : Finset (ZMod 8 × (ZMod 8 × ZMod 11)))
    (hIS : (strongProduct (fractionGraph 8 3)
      (strongProduct (fractionGraph 8 3) (fractionGraph 11 4))).IsIndepSet ↑S)
    (x y : ZMod 8 × (ZMod 8 × ZMod 11))
    (hx : x ∈ S) (hy : y ∈ S) (hxy : x ≠ y)
    (h1 : x.1 = y.1 ∨ (fractionGraph 8 3).Adj x.1 y.1) :
    crossNonAdj83_114' x.2 y.2 = true := by
  rw [crossNonAdj83_114'_spec]
  intro ⟨hc1, hc2⟩
...
private lemma extract_single_from_layer8_88
    (S : Finset (ZMod 8 × (ZMod 8 × ZMod 11))) (i : ZMod 8)
    (hcard : (layerFiber8_88 S i).card = 1) :
    ∃ a : ZMod 8 × ZMod 11, (i, a) ∈ S := by
  obtain ⟨⟨j, a⟩, hja⟩ := Finset.card_pos.mp
    (by omega : 0 < (layerFiber8_88 S i).card)
  rw [layerFiber8_88, Finset.mem_filter] at hja
  exact ⟨a, hja.2 ▸ hja.1⟩

private lemma extract_ordered_pair_from_layer8_88
    (S : Finset (ZMod 8 × (ZMod 8 × ZMod 11))) (i : ZMod 8)
    (hcard : (layerFiber8_88 S i).card = 2) :
    ∃ a b : ZMod 8 × ZMod 11,
      (i, a) ∈ S ∧ (i, b) ∈ S ∧ a ≠ b ∧ enc83_114'_z a < enc83_114'_z b := by
  obtain ⟨⟨i1, a⟩, ⟨i2, b⟩, hne, hfib⟩ := Finset.card_eq_two.mp hcard
  have h1 : (i1, a) ∈ layerFiber8_88 S i := hfib ▸ Finset.mem_insert_self _ _
  have h2 : (i2, b) ∈ layerFiber8_88 S i :=
    hfib ▸ Finset.mem_insert.mpr (Or.inr (Finset.mem_singleton.mpr rfl))
  rw [layerFiber8_88, Finset.mem_filter] at h1 h2
  obtain ⟨h1m, h1e⟩ := h1; obtain ⟨h2m, h2e⟩ := h2
  subst h1e; subst h2e
  have hab : a ≠ b := fun h => hne (Prod.ext rfl h)
  rcases Nat.lt_or_gt_of_ne (fun h => hab (enc83_114'_z_injective h)) with hlt | hgt
...
set_option linter.unusedDecidableInType false in
set_option linter.style.nativeDecide false in
/-- Bound #5 mixed-multiset Baumert contradiction. -/
private lemma case83_83_114_baumert_contradiction
    (S : Finset (ZMod 8 × (ZMod 8 × ZMod 11)))
    ...
    (hfbc : ∀ i : ZMod 8,
      (S.filter
        (fun p => p.1 ∈ ({i, i + 1, i + 2} : Finset (ZMod 8)))).card ≤ 5) :
    False := by
  -- IP rotation: layer sizes are a rotation of (1,2,1,2,2,1,2,2)
  obtain ⟨k, hk⟩ := ip_rotation (fun i => (layerFiber8_88 S i).card)
    (by change ∑ i, (layerFiber8_88 S i).card = 13;
        rw [layer8_88_sum_eq_card]; exact hScard)
    (fun i => triple_bound_sum8_88 S i (hfbc i))
  -- Translate by k on outer Z₈ to get canonical sizes
  let κ : ZMod 8 := k
  set S₁ := S.image (translateFst8_88 κ) with hS₁_def
  have hIS₁ := translateFst8_88_IS κ S hSis
  have hcanon₁ : ∀ j : ZMod 8, (layerFiber8_88 S₁ j).card = canonical_sizes j := by
    intro j
    have h := translateFst8_88_layerFiber κ S (j - κ)
  ...
  -- Extract layer 0 witness from S₁ (size 1)
  obtain ⟨w0₁, hw0₁⟩ := extract_single_from_layer8_88 S₁ 0 (by rw [hcanon₁]; rfl)
  -- Translate inner by δ = (-w0₁.1, -w0₁.2)
  let δ : ZMod 8 × ZMod 11 := (-w0₁.1, -w0₁.2)
  set S' := S₁.image (translateSnd83_114' δ) with hS'_def
  have hIS' := translateSnd83_114'_IS δ S₁ hIS₁
  have hcanon : ∀ j : ZMod 8, (layerFiber8_88 S' j).card = canonical_sizes j := by
    intro j
    rw [translateSnd83_114'_layerFiber δ S₁ j]
    exact hcanon₁ j
  -- Cross-compat
  have hcc := cross_compat_of_IS_83_83_114 S' hIS'
  have hne_layer : ∀ (i j : ZMod 8) (a b : ZMod 8 × ZMod 11),
      i ≠ j → (i, a) ≠ (j, b) :=
    fun _ _ _ _ hij h => hij (congr_arg Prod.fst h)
  have cc : ∀ (i j : ZMod 8) (a b : ZMod 8 × ZMod 11),
      (i, a) ∈ S' → (j, b) ∈ S' → i ≠ j →
      (fractionGraph 8 3).Adj i j →
      crossNonAdj83_114' a b = true :=
    fun i j a b ha hb hij hadj =>
      hcc _ _ ha hb (hne_layer i j a b hij) (Or.inr hadj)
  have cc_eq : ∀ (i : ZMod 8) (a b : ZMod 8 × ZMod 11),
      (i, a) ∈ S' → (i, b) ∈ S' → a ≠ b →
      crossNonAdj83_114' a b = true :=
    fun i a b ha hb hab =>
      hcc _ _ ha hb (fun h => hab (congr_arg Prod.snd h)) (Or.inl rfl)
  have compat_of_list : ∀ (i : ZMod 8) (a : ZMod 8 × ZMod 11)
      (ws : List (ZMod 8 × (ZMod 8 × ZMod 11))),
      (i, a) ∈ S' →
  ...
  obtain ⟨w0, hw0⟩ := extract_single_from_layer8_88 S' 0 (by rw [hcanon]; rfl)
  -- Show w0 = (0, 0)
  have h_zero_in_S' : ((0 : ZMod 8), ((0 : ZMod 8), (0 : ZMod 11))) ∈ S' := by
  ...
  have hfib0 : layerFiber8_88 S' 0 = {(0, w0)} := by
    have hcard0 : (layerFiber8_88 S' 0).card = 1 := by rw [hcanon]; rfl
    have hsubset : ({(0, w0)} : Finset (ZMod 8 × (ZMod 8 × ZMod 11))) ⊆
        layerFiber8_88 S' 0 := by
      intro x hx; simp only [Finset.mem_singleton] at hx; subst hx
      rw [layerFiber8_88, Finset.mem_filter]; exact ⟨hw0, rfl⟩
    have hpair_card : ({(0, w0)} :
        Finset (ZMod 8 × (ZMod 8 × ZMod 11))).card = 1 :=
      Finset.card_singleton _
    exact (Finset.eq_of_subset_of_card_le hsubset
      (by rw [hcard0, hpair_card])).symm
  have h_zero_in_fib : ((0 : ZMod 8), ((0 : ZMod 8), (0 : ZMod 11))) ∈
      layerFiber8_88 S' 0 := by
    rw [layerFiber8_88, Finset.mem_filter]; exact ⟨h_zero_in_S', rfl⟩
  ...
  subst hw0_zero
  -- Extract remaining witnesses
  obtain ⟨w10, w11, hw10, hw11, hne1, hlt1⟩ :=
    extract_ordered_pair_from_layer8_88 S' 1 (by rw [hcanon]; rfl)
  obtain ⟨w2, hw2⟩ := extract_single_from_layer8_88 S' 2
    (by rw [hcanon]; rfl)
  obtain ⟨w30, w31, hw30, hw31, hne3, hlt3⟩ :=
    extract_ordered_pair_from_layer8_88 S' 3 (by rw [hcanon]; rfl)
  obtain ⟨w40, w41, hw40, hw41, hne4, hlt4⟩ :=
    extract_ordered_pair_from_layer8_88 S' 4 (by rw [hcanon]; rfl)
  obtain ⟨w5, hw5⟩ := extract_single_from_layer8_88 S' 5
    (by rw [hcanon]; rfl)
  obtain ⟨w60, w61, hw60, hw61, hne6, hlt6⟩ :=
    extract_ordered_pair_from_layer8_88 S' 6 (by rw [hcanon]; rfl)
  obtain ⟨w70, w71, hw70, hw71, hne7, hlt7⟩ :=
    extract_ordered_pair_from_layer8_88 S' 7 (by rw [hcanon]; rfl)
  -- Adjacency facts in E_{8/3}
  ...
  -- crossNonAdj for Layer 1 (vs layer 0)
  ...
  -- compat conditions
  have cpt_v2 := compat_of_list 2 w2 [(0, ((0:ZMod 8), (0:ZMod 11))), (1, w10), (1, w11)] hw2
  ...
  have cpt_v70 := compat_of_list 7 w70
    [(5, w5), (6, w60), (6, w61),
     (0, ((0:ZMod 8), (0:ZMod 11))), (1, w10), (1, w11)] hw70
    (by intro w hw; simp only [List.mem_cons, List.mem_nil_iff, or_false] at hw
  ...
  have cpt_v71 := compat_of_list 7 w71
    [(5, w5), (6, w60), (6, w61),
     (0, ((0:ZMod 8), (0:ZMod 11))), (1, w10), (1, w11)] hw71
    (by intro w hw; simp only [List.mem_cons, List.mem_nil_iff, or_false] at hw
        rcases hw with rfl | rfl | rfl | rfl | rfl | rfl
        · exact ⟨hw5, fun _ => adj57.symm,
            fun h => by dsimp at h; exact absurd h (by decide)⟩
  ...
  have v0_eq : ((⟨0, by decide⟩, ⟨0, by decide⟩) : Fin 8 × Fin 11) =
      ((0 : ZMod 8), (0 : ZMod 11)) := rfl
  exfalso
  have hf : caseMixed83_83_114_check = false := by
    unfold caseMixed83_83_114_check
    simp only [v0_eq, Bool.not_eq_false']
    apply List.any_of_mem (mem_verts83_114' w10)
    simp only [cn_10_0, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114' w11)
    simp only [show enc83_114' w10 = enc83_114'_z w10 from rfl,
               show enc83_114' w11 = enc83_114'_z w11 from rfl,
               decide_eq_true hlt1, cn_10_11, cn_11_0, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114' w2)
    simp only [cpt_v2, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114' w30)
    simp only [cpt_v30, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114' w31)
    simp only [show enc83_114' w30 = enc83_114'_z w30 from rfl,
               show enc83_114' w31 = enc83_114'_z w31 from rfl,
               decide_eq_true hlt3, cn_30_31, cpt_v31, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114' w40)
    simp only [cpt_v40, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114' w41)
    simp only [show enc83_114' w40 = enc83_114'_z w40 from rfl,
               show enc83_114' w41 = enc83_114'_z w41 from rfl,
               decide_eq_true hlt4, cn_40_41, cpt_v41, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114' w5)
    simp only [cpt_v5, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114' w60)
    simp only [cpt_v60, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114' w61)
    simp only [show enc83_114' w60 = enc83_114'_z w60 from rfl,
               show enc83_114' w61 = enc83_114'_z w61 from rfl,
               decide_eq_true hlt6, cn_60_61, cpt_v61, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114' w70)
    simp only [cpt_v70, Bool.true_and]
    apply List.any_of_mem (mem_verts83_114' w71)
    simp only [show enc83_114' w70 = enc83_114'_z w70 from rfl,
               show enc83_114' w71 = enc83_114'_z w71 from rfl,
               decide_eq_true hlt7, cn_70_71, cpt_v71, Bool.true_and]
  exact absurd caseMixed83_83_114_check_true (by rw [hf]; decide)

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
/-- **Mixed multiset upper bound (bound #5)**:
    α(E_{8/3} ⊠ (E_{8/3} ⊠ E_{11/4})) ≤ 12.

The nested floor bound gives α ≤ 13. The Baumert slicing technique with WLOG
translation by `Z₈ × Z₁₁` rules out α = 13. See `caseMixed83_83_114_check_true`. -/
theorem alpha3_83_83_114_le :
    (strongProduct (fractionGraph 8 3)
  ...
  have hle : (strongProduct (fractionGraph 8 3)
    ...
    have h := nested_floor_three 8 3 8 3 11 4
  ...
  rw [heq] at hSndp
  -- 3-clique fiber bound from α(E_{8/3} ⊠ E_{11/4}) = 5
  have hfbc : ∀ i : ZMod 8,
      (S.filter (fun p =>
  ...
  exact case83_83_114_baumert_contradiction S hSndp.isIndepSet
    hSndp.card_eq hfbc
