...
-/
/-
# Section 6 Baumert Bridge: α(E_{9/4}² ⊠ E_{5/2}) ≤ 8 (alpha3_9o4_9o4_5o2_le)
...
/-- The fiber of S over a single layer i (case 9, Z₉ layers, inner Z₉×Z₅). -/
private def layerFiber9M (S : Finset (ZMod 9 × (ZMod 9 × ZMod 5))) (i : ZMod 9) :=
  S.filter (fun p => p.1 = i)

private lemma layerFiber9M_disjoint (S : Finset (ZMod 9 × (ZMod 9 × ZMod 5)))
    (i j : ZMod 9) (hij : i ≠ j) :
    Disjoint (layerFiber9M S i) (layerFiber9M S j) := by
  rw [Finset.disjoint_left]
  intro x hx hy
  simp only [layerFiber9M, Finset.mem_filter] at hx hy
  exact hij (hx.2 ▸ hy.2)

private lemma layer9M_sum_eq_card (S : Finset (ZMod 9 × (ZMod 9 × ZMod 5))) :
    ∑ i : ZMod 9, (layerFiber9M S i).card = S.card := by
  rw [← Finset.card_biUnion]
  · congr 1
    ext x
    simp only [layerFiber9M, Finset.mem_biUnion, Finset.mem_univ,
      Finset.mem_filter, true_and]
    exact ⟨fun ⟨_, h⟩ => h.1, fun h => ⟨x.1, h, rfl⟩⟩
  · intro i _ j _ hij
    exact layerFiber9M_disjoint S i j hij

private lemma quadruple_filter_eq9 (S : Finset (ZMod 9 × (ZMod 9 × ZMod 5)))
    (i : ZMod 9) :
    S.filter (fun p =>
      p.1 ∈ ({i, i + 1, i + 2, i + 3} : Finset (ZMod 9))) =
    layerFiber9M S i ∪ layerFiber9M S (i + 1) ∪
      layerFiber9M S (i + 2) ∪ layerFiber9M S (i + 3) := by
  ext x; simp only [layerFiber9M, Finset.mem_filter, Finset.mem_union,
    Finset.mem_insert, Finset.mem_singleton]
  tauto

set_option linter.style.nativeDecide false in
/-- Consecutive offsets 0,1,2,3 are pairwise distinct in ZMod 9. -/
private lemma four_distinct_zmod9 (i : ZMod 9) :
    i ≠ i + 1 ∧ i ≠ i + 2 ∧ i ≠ i + 3 ∧
    (i + 1 : ZMod 9) ≠ i + 2 ∧ (i + 1 : ZMod 9) ≠ i + 3 ∧
    (i + 2 : ZMod 9) ≠ i + 3 := by
  revert i; decide

private lemma quadruple_bound_sum9
    (S : Finset (ZMod 9 × (ZMod 9 × ZMod 5))) (i : ZMod 9)
    (hfbc : (S.filter (fun p =>
      p.1 ∈ ({i, i + 1, i + 2, i + 3} : Finset (ZMod 9)))).card ≤ 4) :
    (layerFiber9M S i).card + (layerFiber9M S (i + 1)).card +
    (layerFiber9M S (i + 2)).card + (layerFiber9M S (i + 3)).card ≤ 4 := by
  rw [quadruple_filter_eq9] at hfbc
  obtain ⟨h01, h02, h03, h12, h13, h23⟩ := four_distinct_zmod9 i
  -- Disjointness of unions
  have d12 : Disjoint (layerFiber9M S i) (layerFiber9M S (i + 1)) :=
    layerFiber9M_disjoint S _ _ h01
  have h_union12 := Finset.card_union_of_disjoint d12
  have d3_with_12 :
      Disjoint (layerFiber9M S i ∪ layerFiber9M S (i + 1))
        (layerFiber9M S (i + 2)) := by
    rw [Finset.disjoint_union_left]
    exact ⟨layerFiber9M_disjoint S _ _ h02,
      layerFiber9M_disjoint S _ _ h12⟩
  ...
  have d4_with_123 :
    ...
    refine ⟨⟨?_, ?_⟩, ?_⟩
    · exact layerFiber9M_disjoint S _ _ h03
    · exact layerFiber9M_disjoint S _ _ h13
    · exact layerFiber9M_disjoint S _ _ h23
...
private lemma ip_uniform94_zmod_fin5 :
    ∀ s : ZMod 9 → Fin 5,
...
private lemma ip_uniform94 (s : ZMod 9 → ℕ)
    (hsum : ∑ i : ZMod 9, s i = 9)
    (hquad : ∀ i : ZMod 9,
  ...
  intro i
  have := ip_uniform94_zmod_fin5 s' hsum' hquad' i
  simp only [s'] at this; exact this

/-- Translate S by shifting the inner two coordinates (case 9, Z₉ × Z₅). -/
private def translateSnd94_52 (δ : ZMod 9 × ZMod 5)
    (x : ZMod 9 × (ZMod 9 × ZMod 5)) : ZMod 9 × (ZMod 9 × ZMod 5) :=
  (x.1, (x.2.1 + δ.1, x.2.2 + δ.2))

private lemma translateSnd94_52_injective (δ : ZMod 9 × ZMod 5) :
    Function.Injective (translateSnd94_52 δ) := by
  intro ⟨a1, a2, a3⟩ ⟨b1, b2, b3⟩ h
  simp only [translateSnd94_52, Prod.mk.injEq] at h
  obtain ⟨h1, h2, h3⟩ := h
  exact Prod.ext h1 (Prod.ext (add_right_cancel h2) (add_right_cancel h3))

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
/-- Inner-coordinate translation preserves the IS property (case 9). -/
private lemma translateSnd94_52_IS (δ : ZMod 9 × ZMod 5)
    (S : Finset (ZMod 9 × (ZMod 9 × ZMod 5)))
    (hIS : (strongProduct (fractionGraph 9 4)
      (strongProduct (fractionGraph 9 4) (fractionGraph 5 2))).IsIndepSet ↑S) :
    (strongProduct (fractionGraph 9 4)
      (strongProduct (fractionGraph 9 4) (fractionGraph 5 2))).IsIndepSet
      ↑(S.image (translateSnd94_52 δ)) := by
  ...
  simp only [translateSnd94_52] at hne hadj
  ...
  have h_inner_back : (a2, a3) = (b2, b3) ∨ (strongProduct (fractionGraph 9 4)
      (fractionGraph 5 2)).Adj (a2, a3) (b2, b3) := by
    ...
    · right
      ...
      · exact h3_or.imp (fun h => add_right_cancel h)
          (fractionGraph_adj_translate 5 2 a3 b3 δ.2).mp
  exact (SimpleGraph.isIndepSet_iff _).mp hIS
    (Finset.mem_coe.mpr ha) (Finset.mem_coe.mpr hb) hne'
    ⟨hne', h_first, h_inner_back⟩

private lemma translateSnd94_52_layerFiber (δ : ZMod 9 × ZMod 5)
    (S : Finset (ZMod 9 × (ZMod 9 × ZMod 5))) (i : ZMod 9) :
    (layerFiber9M (S.image (translateSnd94_52 δ)) i).card =
    (layerFiber9M S i).card := by
  have : layerFiber9M (S.image (translateSnd94_52 δ)) i =
      (layerFiber9M S i).image (translateSnd94_52 δ) := by
    ext ⟨j, a, b⟩
    simp only [layerFiber9M, Finset.mem_filter, Finset.mem_image, translateSnd94_52]
    constructor
  ...
  rw [this, Finset.card_image_of_injective _ (translateSnd94_52_injective δ)]

set_option linter.style.nativeDecide false in
/-- Every element of `Fin 9 × Fin 5` is in `verts94_52`. -/
private lemma mem_verts94_52' (v : Fin 9 × Fin 5) : v ∈ verts94_52 := by
  revert v; native_decide

set_option linter.style.nativeDecide false in
/-- Bridge: `crossNonAdj94_52 a b = true` iff the formal adj-or-eq condition fails
    for E_{9/4} ⊠ E_{5/2}. -/
private lemma crossNonAdj94_52_spec (a b : ZMod 9 × ZMod 5) :
    crossNonAdj94_52 a b = true ↔
...
set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
/-- From IS property and first-coordinate adj-or-eq, derive crossNonAdj94_52. -/
private lemma cross_compat_of_IS_94_52
    (S : Finset (ZMod 9 × (ZMod 9 × ZMod 5)))
    (hIS : (strongProduct (fractionGraph 9 4)
      (strongProduct (fractionGraph 9 4) (fractionGraph 5 2))).IsIndepSet ↑S)
    (x y : ZMod 9 × (ZMod 9 × ZMod 5))
    (hx : x ∈ S) (hy : y ∈ S) (hxy : x ≠ y)
    (h1 : x.1 = y.1 ∨ (fractionGraph 9 4).Adj x.1 y.1) :
    crossNonAdj94_52 x.2 y.2 = true := by
  rw [crossNonAdj94_52_spec]
  intro ⟨hc1, hc2⟩
  have hnadj : ¬(strongProduct (fractionGraph 9 4)
      (strongProduct (fractionGraph 9 4) (fractionGraph 5 2))).Adj x y :=
...
private lemma extract_single_from_layer9M
    (S : Finset (ZMod 9 × (ZMod 9 × ZMod 5))) (i : ZMod 9)
    (hcard : (layerFiber9M S i).card = 1) :
    ∃ a : ZMod 9 × ZMod 5, (i, a) ∈ S := by
  obtain ⟨⟨j, a⟩, hja⟩ := Finset.card_pos.mp
    (by omega : 0 < (layerFiber9M S i).card)
  rw [layerFiber9M, Finset.mem_filter] at hja
  exact ⟨a, hja.2 ▸ hja.1⟩

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
set_option linter.style.nativeDecide false in
/-- Case 9 mixed-multiset Baumert contradiction: an IS of size 9 in
    E_{9/4} ⊠ E_{9/4} ⊠ E_{5/2} satisfying the quadruple-fiber bound 4
    cannot exist. -/
private lemma case94_baumert_contradiction
    (S : Finset (ZMod 9 × (ZMod 9 × ZMod 5)))
  ...
  have hsize : ∀ j : ZMod 9, (layerFiber9M S j).card = 1 := by
    apply ip_uniform94 (fun i => (layerFiber9M S i).card)
    · rw [layer9M_sum_eq_card]; exact hScard
    · intro i; exact quadruple_bound_sum9 S i (hfbc i)
  -- Extract the unique witness in layer 0
  obtain ⟨w0₁, hw0₁⟩ := extract_single_from_layer9M S 0 (hsize 0)
  -- Translate so that w0 = (0, 0): inner shift δ = (-w0₁.1, -w0₁.2)
  let δ : ZMod 9 × ZMod 5 := (-w0₁.1, -w0₁.2)
  set S' := S.image (translateSnd94_52 δ) with hS'_def
  have hIS' := translateSnd94_52_IS δ S hSis
  have hsize' : ∀ j : ZMod 9, (layerFiber9M S' j).card = 1 := by
    intro j
    rw [translateSnd94_52_layerFiber δ S j]
    exact hsize j
  -- Cross-compat
  have hcc := cross_compat_of_IS_94_52 S' hIS'
  have hne_layer : ∀ (i j : ZMod 9) (a b : ZMod 9 × ZMod 5),
      i ≠ j → (i, a) ≠ (j, b) :=
    fun _ _ _ _ hij h => hij (congr_arg Prod.fst h)
  have cc : ∀ (i j : ZMod 9) (a b : ZMod 9 × ZMod 5),
      (i, a) ∈ S' → (j, b) ∈ S' → i ≠ j →
  ...
  have cc_eq : ∀ (i : ZMod 9) (a b : ZMod 9 × ZMod 5),
      (i, a) ∈ S' → (i, b) ∈ S' → a ≠ b →
      crossNonAdj94_52 a b = true :=
    fun i a b ha hb hab =>
      hcc _ _ ha hb (fun h => hab (congr_arg Prod.snd h)) (Or.inl rfl)
  -- compatWith94_52 via cc and cc_eq (case 7 / case 8 style)
  have compat_of_list : ∀ (i : ZMod 9) (a : ZMod 9 × ZMod 5)
      (ws : List (ZMod 9 × (ZMod 9 × ZMod 5))),
      (i, a) ∈ S' →
      (∀ w ∈ ws, w ∈ S' ∧ (i ≠ w.1 → (fractionGraph 9 4).Adj i w.1) ∧
        (i = w.1 → a ≠ w.2)) →
      compatWith94_52 a (ws.map Prod.snd) = true := by
    intro i a ws hia hws
    simp only [compatWith94_52, List.all_eq_true, List.mem_map]
  ...
  have h_zero_in_S' : ((0 : ZMod 9), ((0 : ZMod 9), (0 : ZMod 5))) ∈ S' := by
    rw [hS'_def, Finset.mem_image]
    refine ⟨(0, w0₁), hw0₁, ?_⟩
    simp only [translateSnd94_52, δ]
    ext
    · rfl
    · change w0₁.1 + (- w0₁.1) = 0; ring
    · change w0₁.2 + (- w0₁.2) = 0; ring
  have hfib0 : layerFiber9M S' 0 = {(0, w0)} := by
    have hcard0 : (layerFiber9M S' 0).card = 1 := hsize' 0
    have hsubset : ({(0, w0)} : Finset (ZMod 9 × (ZMod 9 × ZMod 5))) ⊆
        layerFiber9M S' 0 := by
      intro x hx
      simp only [Finset.mem_singleton] at hx
      subst hx
      rw [layerFiber9M, Finset.mem_filter]; exact ⟨hw0, rfl⟩
    have hpair_card : ({(0, w0)} : Finset (ZMod 9 × (ZMod 9 × ZMod 5))).card = 1 :=
      Finset.card_singleton _
    exact (Finset.eq_of_subset_of_card_le hsubset (by rw [hcard0, hpair_card])).symm
  have h_zero_in_fib : ((0 : ZMod 9), ((0 : ZMod 9), (0 : ZMod 5))) ∈
      layerFiber9M S' 0 := by
    rw [layerFiber9M, Finset.mem_filter]; exact ⟨h_zero_in_S', rfl⟩
  ...
  obtain ⟨w1, hw1⟩ := extract_single_from_layer9M S' 1 (hsize' 1)
  obtain ⟨w2, hw2⟩ := extract_single_from_layer9M S' 2 (hsize' 2)
  obtain ⟨w3, hw3⟩ := extract_single_from_layer9M S' 3 (hsize' 3)
  obtain ⟨w4, hw4⟩ := extract_single_from_layer9M S' 4 (hsize' 4)
  obtain ⟨w5, hw5⟩ := extract_single_from_layer9M S' 5 (hsize' 5)
  obtain ⟨w6, hw6⟩ := extract_single_from_layer9M S' 6 (hsize' 6)
  obtain ⟨w7, hw7⟩ := extract_single_from_layer9M S' 7 (hsize' 7)
  obtain ⟨w8, hw8⟩ := extract_single_from_layer9M S' 8 (hsize' 8)
  ...
  -- Show caseMixed94_check = false, contradicting caseMixed94_check_true.
  -- The hardcoded s0 = (0, 0) in caseMixed94_check matches w0 = (0, 0) here.
  have s0_eq : ((⟨0, by decide⟩, ⟨0, by decide⟩) : Fin 9 × Fin 5) =
      ((0 : ZMod 9), (0 : ZMod 5)) := rfl
  exfalso
  have hf : caseMixed94_check = false := by
    unfold caseMixed94_check
    simp only [s0_eq, Bool.not_eq_false']
    apply List.any_of_mem (mem_verts94_52 w1)
    simp only [cn_1_0, Bool.true_and]
    apply List.any_of_mem (mem_verts94_52 w2)
    simp only [cpt_2, Bool.true_and]
    apply List.any_of_mem (mem_verts94_52 w3)
    simp only [cpt_3, Bool.true_and]
    apply List.any_of_mem (mem_verts94_52 w4)
    simp only [cpt_4, Bool.true_and]
    apply List.any_of_mem (mem_verts94_52 w5)
    simp only [cpt_5, Bool.true_and]
    apply List.any_of_mem (mem_verts94_52 w6)
    simp only [cpt_6, Bool.true_and]
    apply List.any_of_mem (mem_verts94_52 w7)
    simp only [cpt_7, Bool.true_and]
    apply List.any_of_mem (mem_verts94_52 w8)
    simp only [cpt_8]
  exact absurd caseMixed94_check_true (by rw [hf]; decide)

set_option linter.unusedFintypeInType false in
set_option linter.unusedDecidableInType false in
/-- **Mixed multiset upper bound (case 9)**:
    α(E_{9/4} ⊠ (E_{9/4} ⊠ E_{5/2})) ≤ 8.

The nested floor bound gives α ≤ 9. The Baumert slicing technique with WLOG
translation by `Z₉ × Z₅` (a symmetry of E_{9/4} ⊠ E_{5/2}) bringing the unique
element of layer 0 to `(0, 0)` rules out α = 9: the IP forces all 9 layer
sizes to be 1, and direct layer-assignment search over E_{9/4} ⊠ E_{5/2} with
`s0 = (0, 0)` finds no valid configuration. See `caseMixed94_check_true`. -/
theorem alpha3_9o4_9o4_5o2_le :
    (strongProduct (fractionGraph 9 4)
      (strongProduct (fractionGraph 9 4) (fractionGraph 5 2))).indepNum ≤ 8 := by
  by_contra hge; push_neg at hge
  have hle : (strongProduct (fractionGraph 9 4)
      (strongProduct (fractionGraph 9 4) (fractionGraph 5 2))).indepNum ≤ 9 := by
    have h := nested_floor_three 9 4 9 4 5 2
  ...
  have heq : (strongProduct (fractionGraph 9 4)
      (strongProduct (fractionGraph 9 4) (fractionGraph 5 2))).indepNum = 9 := by omega
  obtain ⟨S, hSndp⟩ := SimpleGraph.exists_isNIndepSet_indepNum
    (G := strongProduct (fractionGraph 9 4)
      (strongProduct (fractionGraph 9 4) (fractionGraph 5 2)))
  rw [heq] at hSndp
  -- Quadruple-fiber bound from α(E_{9/4} ⊠ E_{5/2}) = 4
  have hfbc : ∀ i : ZMod 9,
      (S.filter (fun p =>
    ...
    rwa [alpha_9o4_5o2] at h
  exact case94_baumert_contradiction S hSndp.isIndepSet hSndp.card_eq hfbc
